...
2.  **Strict Upper Bound:** CC(S) is bounded. As derived later (Theorem 39), avoidance of deterministic endpoint forcing imposes $\alpha_{CC,max}<0.5$. The stronger statistical-FTL consistency condition is the finite-window zero-error gate of Theorem 39a on the regular branch. Thus, CC($C_{agg}$) $\le \alpha_{CC,max}<0.5$ for all finite $C_{agg}$ on the bounded-bias branch.

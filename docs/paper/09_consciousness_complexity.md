...
**Conjecture boundary.** CC is a model-level extension of the core MPU framework. Postulate 3 is presented in two branches: (i) a local CPTP branch on which ordinary no-signaling is preserved and only joint/conditional correlations after classical comparison can vary; and (ii) a nonlocal/state-mediated branch on which Alice's context can produce a spacelike-correlated shift of Bob's marginal $P(b)$. The second branch is a genuine *statistical FTL influence* in the precise technical sense — an Alice-context-dependent shift of Bob's local statistics across spacelike separation — and is a real departure from standard quantum mechanics, not merely a rewording of ordinary nonlocal correlations. It is non-signaling for deterministic purposes by Theorem 39 ($\mathrm{CC}<0.5$) and by the sample-complexity and information-rate bounds of Theorems 40–42, but it is not the same hypothesis as the local CPTP branch. The protocols in Sections 10 and 13 are interpreted as tests of a statistical FTL influence model on the nonlocal/state-mediated branch, with the local CPTP branch serving as the no-signaling reference; neither branch is a theorem-level consequence of the core quantum-emergence theorem.

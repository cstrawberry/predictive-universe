...
**CC conjecture boundary.** CC is a model-level extension of the core MPU framework. Postulate 3 is presented in three operationally distinct branches: (i) a local CPTP branch on which ordinary no-signaling is preserved and Bob's unconditional marginal is invariant, while joint/conditional correlations after classical comparison can vary; (ii) a preparation-context branch on which different global states $\omega_C$ are prepared in the shared causal past of Alice and Bob, so that Bob's marginal $P(b)$ may depend on the context label $C$ through common-cause statistics with no spacelike action by Alice after separation; and (iii) a nonlocal/state-mediated marginal-anomaly branch on which Alice's local context choice, made after the entangled state has been distributed and after the relevant shared-past preparation event, induces a spacelike-correlated shift of Bob's marginal $P(b)$. Branch (iii) is a genuine *statistical FTL influence* in the precise technical sense — an Alice-context-dependent shift of Bob's local statistics across spacelike separation, with Alice's choice fixed strictly later than the latest event in the shared causal past of the two measurement regions — and is a real departure from standard quantum mechanics, not merely a rewording of ordinary nonlocal correlations or preparation-context dependence. It is outside operational no-signaling by Theorem 39c whenever the context is independently selectable; its zero-error reliability is nevertheless bounded by the endpoint bound of Theorem 39, the finite-window zero-error gate of Theorem 39a on the regular statistical branch, and the sample-complexity and information-rate bounds of Theorems 40–42; it is not reduced to the Bob-marginal-preserving branch (i) or the shared-past branch (ii). The protocols in Sections 10 and 13 distinguish the three branches by experimental design: branch (i) admits only post-comparison joint-correlation or Alice-local effects; branch (ii) is excluded as an explanation of Bob-marginal shifts only by late randomization of Alice's context strictly after the latest event in the shared causal past of the two measurement regions; branch (iii) is the unique branch on which a Bob-marginal shift remains under late randomization; by Theorem 39c and Corollary 39c.1, it is absent from—and would falsify—the exact pre-lightcone context-independence branch. None of the three branches is a theorem-level consequence of the core quantum-emergence theorem.

The framework developed thus far describes the fundamental constituents (MPUs) and their dynamics, leading to the emergence of quantum mechanical formalism. We now investigate the behavior of complex systems formed from aggregates of these MPUs. This section introduces the Consciousness Complexity (CC) hypothesis, proposing that sufficiently complex MPU aggregates, driven by the core optimization principles (POP/PCE), can under specific operational conditions develop an information-geometric capability to bias the probabilities of local 'Evolve' outcomes by small amounts. Theorems 39 and 39a cap endpoint forcing and regular finite-window zero-error decoding; they do not establish Postulate 2. By Theorem 39c, a freely selected late context that changes a pre-lightcone marginal is signaling, so Postulate 3(iii) is explicitly outside the causal branch and, by Corollary 39c.1, is its preregistered falsifier class. CC provides a specific, physically constrained mechanism linking complex predictive processing to quantum phenomena.
...
We define Consciousness Complexity (CC) operationally as the quantitative measure of the emergent biasing capability conditionally certified by Theorem 34. The word "observer" denotes any finite record-forming protocol in the measurement formalism; CC is not an additional gauge group and is not a prerequisite for Evolve. High-CC systems enter the quantum formalism as high-resource record-integrating and context-modulating subsystems, while inter-perspective agreement of durable records is handled by Theorem M.4a in Appendix M.
...
2.  **Strict Upper Bound:** CC(S) is bounded. As derived later (Theorem 39), avoidance of deterministic endpoint forcing imposes $\alpha_{CC,max}<0.5$. Theorem 39a gives only a finite-window zero-error bound; the causal-compliance condition is exact marginal invariance by Theorem 39c. Thus, CC($C_{agg}$) $\le \alpha_{CC,max}<0.5$ for all finite $C_{agg}$ on the bounded-bias branch.

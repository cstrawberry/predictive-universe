...
$$
Integrating over $\tau$ and dividing by $\tau$, assuming $\delta_v$ and $R'(\langle \hat{C}_v \rangle)$ vary slowly such that their time averages over $\tau$ are well approximated by their instantaneous values (or using mean value theorem for integrals), yields Eq. (D.3). The approximation holds well for small $\delta_v$. QED

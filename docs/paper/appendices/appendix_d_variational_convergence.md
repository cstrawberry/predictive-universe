...
$$
where:
*   **$V_{op}(x) = \sum_{v} \langle \lambda \hat{R}_v(C_v) + \hat{R}_{I,v}(C_v) \rangle_{\rho(x)}$**: The total expected operational resource cost rate. $\hat{R}_v = R(\hat{C}_v)$ and $\hat{R}_{I,v} = R_I(\hat{C}_v)$ are the cost operators derived from the operational complexity proxy $\hat{C}_v$ (Theorem 3, Equation B.3), $\lambda$ is the scarcity factor (Definition 20), and the expectation $\langle \cdot \rangle_{\rho(x)}$ is taken with respect to the system state $\rho(x)$ corresponding to configuration $x$. This term reflects the ongoing cost of maintaining complexity and processing information based on the system's *operational* estimate of complexity $C_v = \langle \hat{C}_v \rangle_x$.
...
This focused analysis using Lyapunov methods and LaSalle's principle for the deterministic part, along with standard stochastic approximation results, robustly supports the conclusion that the system dynamically drives its complexity $C(t)$ towards the unique POP-optimal value $C^{\star}$ (Proposition I.1), even in the presence of operational noise. This is consistent with and provides a more detailed mechanism for one aspect of the global convergence proven more generally for $V(x)$ in Section D.6.5 (Theorem D.5).

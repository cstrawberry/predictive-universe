...
*   Let $\sigma_{geom_link} = 1/(\eta \delta^2)$ be the purely geometric surface density of potential links based on MPU spacing $\delta$ and packing factor $\eta$ (where $\eta$ is an $\mathcal{O}(1)$ factor, e.g., $\eta=1$ for simple cubic packing normal to surface).

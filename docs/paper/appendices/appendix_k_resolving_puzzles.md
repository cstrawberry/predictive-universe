...
**Theorem K.6.11 (Strong CP Resolution on the σ-Symmetric Positive-Orientation Branch).** On the branch satisfying the σ-symmetric PCE-attractor condition of Theorem K.6.6 and one of the positive-orientation real-Yukawa certificates of Corollary K.6.5 in both quark sectors,
$$
\bar\theta
=
...
*Proof.* Theorem K.6.6 gives $\theta_{\text{QCD}}=0$ on the σ-symmetric PCE-attractor branch. Corollary K.6.5 gives $\arg(\det M_q)=0$ on any certified positive-orientation real-Yukawa branch. Therefore $\bar\theta=0+0=0$. Without a positive-orientation determinant certificate, $E_8$ reality gives real Yukawa data but does not exclude the negative determinant component, so the theorem is branch-level exactly as stated. ∎

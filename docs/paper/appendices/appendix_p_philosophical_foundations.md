...
**Definition (Regress-Terminating Ground).** A proposition $p$ is a *regress-terminating ground* for $Q_{\mathrm{PU}}$ if and only if: (i) $p$ can be offered as an explanation for why $\mathcal{U}$ exists rather than not, and (ii) $p$'s own existence or truth does not itself generate a contrastive existence question of the form "why does $p$ obtain rather than not?" that is well-posed and unresolved. Condition (ii) is satisfied precisely when $p$ requires no further ground — i.e.\ when $p$ is self-certifying under the doubt procedure.
...
$Q_{\mathrm{PU}}$ is the canonical formulation of $Q$ from the Cogito vantage point — the tightest the framework licenses, with $Q_{\mathrm{strict}}$ available as a weaker but equally consistent precursor. $Q_{\mathrm{PU}}$ inherits the internal closure already proved for $Q$ and is additionally closed externally by Proposition P.3.5.9.2. Since Theorem P.3.5.1 establishes that any answerer is contained within $\mathcal{U}$, the inside/outside dichotomy is exhaustive: there is no third position from which $Q_{\mathrm{PU}}$ could be addressed. The question is meaningful, the binary is real, and the impossibility is complete.

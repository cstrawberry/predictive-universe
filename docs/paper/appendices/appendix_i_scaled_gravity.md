...
*   **Information limits:** ND-RID is inherently irreversible ($\varepsilon\!\ge\!\ln2$, Theorem 31), and the completed SPAP reset imposes the reset-support capacity deficit $C_{\max}\le\ln d_0-\ln2$ (Proposition E.2a in Appendix E). On refresh/minorization branches, the averaged 'Evolve' channel includes a nonzero input-independent full-state refresh component (Lemma E.1), yielding strict trace-distance contraction ($f_{\mathrm{RID}}\!<\!1$) and therefore a strict capacity bound $C_{\max}(f_{\mathrm{RID}})\!<\!\ln d_{0}$ (Theorem E.2 in Appendix E).

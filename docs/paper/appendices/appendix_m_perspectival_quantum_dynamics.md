...
During the Internal Prediction phase (Definition 26), the state amplitude $S(t)=|\psi(t)\rangle$ evolves deterministically. On the Hilbert/Born branch satisfying the continuity and time-translation hypotheses of Theorem 8.7, this deterministic reset-free branch is implemented unitarily and obeys the time-dependent Schrödinger equation generated by the MPU's internal Hamiltonian $\hat H$ (identified with baseline operational energy cost $R(C_{op})$, Theorem 29):
$$
...
- **Epistemic domain** (Branch I): SPAP $\to$ Logical Indeterminacy $\to$ intrinsic transition kernels in the PPI quotient (Theorem 28) $\to$ Born-rule outcome weights on the Hilbert branch $\to$ definite perspective-indexed actualization after 'Evolve' $\to$ perspective-relative actuality. The completed reset branch supplies the entropy and finite-transfer ledger for actualization; refresh/minorization subbranches additionally give $f_{RID}<1$ and strict reduced-state contraction, which strengthens disturbance and mixing but is not the source of perspectival actuality.
...
1. an absolute $G$-label with zero frameness changes no retained finite protocol and is removed by PPI/PCE;

2. a relative phase, charge, orientation, or perspective label survives only as a finite resource that can be consumed, transferred, or bounded in an admissible protocol;

3. the charge-covariant measurement bound of Theorem M.6.13b is the conserved-charge instance of the same frameness ledger.

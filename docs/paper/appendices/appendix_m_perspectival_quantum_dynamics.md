...
- $g: (0, \alpha_{SPAP}] \to (0, \infty)$ is monotone decreasing with $g(0^+) = 0$, encoding the requirement that smaller gaps to the SPAP boundary demand more precise self-model agreement. The linear choice $g(\delta) = \delta$ suffices for all results below; any monotone decreasing $g$ with $g(0^+) = 0$ yields the same qualitative structure, since the asymptotic divergence class (Theorem M.10.3) inherits from Theorem 14 independently of $g$.

The infimum exists because $[0, \alpha_{SPAP}]$ is compact and the constraint set $\{PP : \|\Pi_S^{(PP)}(\theta_S') - \theta_S'\|_{\mathcal{F}_S} \leq g(\alpha_{SPAP} - PP)\}$ is closed from below (since the left side is continuous in $PP$ by smoothness of $\Pi_S^{(PP)}$, $g$ is monotone decreasing and continuous from the right, and the sublevel set of a continuous function is closed). If the constraint set is empty — no sub-boundary performance suffices — define $PP_S^{(E)} := \alpha_{SPAP}$.
...
**Remark M.10.3.** The three components are not independent. High $\sigma_S$ (high reflexivity fraction) tends to produce high $\mu_S$ (high SPAP proximity) because self-model modifications engage the SPAP mechanism. But the relationship is not monotonic: a pattern can have high reflexivity and low SPAP proximity if the self-model change is shallow (e.g., updating a trivial self-assessment), and conversely a pattern with moderate reflexivity can have high SPAP proximity if it targets deep self-model parameters (e.g., a critique of fundamental methodology). SPAP proximity tracks depth of self-referential engagement, not merely the fraction of change directed at the self-model.
...
*Proof.* Let $S_1$ and $S_2$ differ in self-model parameter $\theta_{S_1}^{(k)} \neq \theta_{S_2}^{(k)}$. Construct a pattern $E$ asserting: "Your parameter $\theta^{(k)}$ is incorrect." For $S_1$, processing $E$ requires modifying $\theta_{S_1}^{(k)}$, yielding $\|\Delta M_{S_1}^{(\text{self})}(E)\|_{\mathcal{F}_{S_1}} > 0$ and hence $\sigma_{S_1}(E) > 0$. The self-consistency condition (Equation M.18) is engaged, producing $PP_{S_1}^{(E)} > 0$ and hence $\mu_{S_1}(E) > 1/\alpha_{SPAP}$. For $S_2$, if $\theta_{S_2}^{(k)}$ already satisfies the assertion, processing $E$ requires no self-model change: $\sigma_{S_2}(E) = 0$ and $\mu_{S_2}(E) = 1/\alpha_{SPAP}$ (baseline). Since $\mu_{S_1}(E) > 1/\alpha_{SPAP} = \mu_{S_2}(E)$, the profiles differ. $\square$
...
*Proof.* Let $E_1$ and $E_2$ both be binary propositions (true/false with equal prior probability), so $H(E_1) = H(E_2) = \ln 2$. Let $E_1$ = "It will rain tomorrow." For any system $S$ with a non-trivial self-model: $E_1$ modifies only the external model, giving $\sigma_S(E_1) = 0$ and $\mu_S(E_1) = 1/\alpha_{SPAP}$ (baseline).

For $E_2$, use the binary diagonal construction: let $E_2$ encode a prediction about a single diagonal system $S_{diag}$ (Equation 10) constructed against one binary self-model register of $S$. This is a binary proposition ($H = \ln 2$) that engages the self-model ($\sigma_S(E_2) > 0$). By SPAP (Theorem 10), for every $PP < \alpha_{SPAP}$, the discrepancy $\|\Pi_S^{(PP)}(\theta_{S,2}') - \theta_{S,2}'\|_{\mathcal{F}_S} \geq D_1 > 0$ on the diagonal register, where $D_1$ is the minimum Fisher-norm discrepancy guaranteed by the fixed-point-free property of the diagonal. Since $g(\alpha_{SPAP} - PP) \to 0$ as $PP \to \alpha_{SPAP}$, there exists $PP_0 > 0$ such that $g(\alpha_{SPAP} - PP_0) < D_1$. For all $PP > PP_0$, the constraint $\|\Pi_S^{(PP)}(\theta_{S,2}') - \theta_{S,2}'\|_{\mathcal{F}_S} \leq g(\alpha_{SPAP} - PP)$ fails, so $PP_S^{(E_2)} \geq PP_0 > 0$. Hence $\delta_S(E_2) = \alpha_{SPAP} - PP_S^{(E_2)} < \alpha_{SPAP}$ and $\mu_S(E_2) > 1/\alpha_{SPAP}$.

Since $H(E_1) = H(E_2) = \ln 2$ but $\mu_S(E_1) = 1/\alpha_{SPAP} < \mu_S(E_2)$, the SPAP proximity varies independently of Shannon entropy. $\square$
...
**Theorem M.10.4 (Existence of Divergent SPAP Proximity).** For every system $S$ with Effective Operational Property R, there exist patterns $E^*$ such that $\mu_S(E^*) = \infty$.
...
**Corollary M.10.4.1 (Full Range of SPAP Proximity).** Every predictive system $S$ with Effective Operational Property R necessarily inhabits a *perspectival landscape*: a space of patterns with $\mu_S$ values spanning $[1/\alpha_{SPAP}, \infty]$. The lower bound $\mu_S = 1/\alpha_{SPAP}$ is achieved by any pattern with $\sigma_S = 0$ (Corollary M.10.3.1). The value $\mu_S = \infty$ is achieved by Theorem M.10.4. For intermediate values, define a one-parameter family $\{E_\lambda\}_{\lambda \in [0,1]}$ where $E_0$ is a purely external pattern with $\sigma_S(E_0) = 0$ and $E_1 = E^*$ is the pattern of Theorem M.10.4.

For $\lambda \in (0,1)$, let $E_\lambda$ assign probability $\lambda$ to the content of $E^*$ and probability $1-\lambda$ to the content of $E_0$. The self-model perturbation $\delta\theta_S(E_\lambda)$ is determined by the expected model-change, which depends linearly on $\lambda$: since $\delta\theta_S(E_0) = 0$ (purely external) and $\delta\theta_S(E^*) = \delta\theta^*$, linearity of expectation gives $\delta\theta_S(E_\lambda) = \lambda \cdot \delta\theta^*$. Continuity of the self-model prediction map $\Pi_S^{(PP)}$ in its arguments (smoothness on the finite-dimensional manifold $[0, \alpha_{SPAP}] \times \Theta_S$, as specified in Definition M.10.3) implies that the discrepancy function $D(\lambda, PP) := \|\Pi_S^{(PP)}(\theta_S + \lambda\delta\theta^*) - (\theta_S + \lambda\delta\theta^*)\|_{\mathcal{F}_S}$ is jointly continuous in $(\lambda, PP)$. The constraint set $\mathcal{C}(\lambda) := \{PP \in [0, \alpha_{SPAP}] : D(\lambda, PP) \leq g(\alpha_{SPAP} - PP)\}$ therefore varies continuously with $\lambda$ in the Hausdorff metric on closed subsets of $[0, \alpha_{SPAP}]$ (by continuity of $D$ and $g$). The infimum $PP_S^{(E_\lambda)} = \inf \mathcal{C}(\lambda)$ (or $\alpha_{SPAP}$ if $\mathcal{C}(\lambda) = \emptyset$) is a non-decreasing function of $\lambda$: increasing $\lambda$ increases $\|\delta\theta_S\|$ and hence the discrepancy at each $PP$, which can only shrink or empty the constraint set, raising $PP_S^{(E_\lambda)}$. A non-decreasing function from $[0,1]$ to $[1/\alpha_{SPAP}, \infty]$ that achieves both endpoints (at $\lambda = 0$ and $\lambda = 1$) achieves every intermediate value, since the only way for a non-decreasing function to skip a value in an interval is via a downward jump, which is precluded. Therefore every value in $(1/\alpha_{SPAP}, \infty)$ is achieved. $\square$

**Theorem M.10.5 (Downward Measurability — Measurement Asymmetry).** Let $A, B$ be predictive systems with $C_{agg}(A) > C_{agg}(B) > C_{op}$, both possessing Effective Operational Property R. Then:

(i) $A$ can compute $\mu_B(E)$ for any pattern $E$ by externally modeling $B$'s self-model $\mathcal{M}_B$. Since $C_{agg}(A) > C_{agg}(B)$, $A$ has sufficient complexity to represent $\mathcal{M}_B$ as an external object within $A$'s model space (Property R, Definition 10). $A$ then computes the decomposition (Equation M.17) for $E$ relative to $B$, determines $PP_B^{(E)}$ (Definition M.10.3) by evaluating $B$'s self-consistency condition externally, and evaluates $\mu_B(E) = 1/(\alpha_{SPAP} - PP_B^{(E)})$. This computation concerns $B$'s self-model, not $A$'s. Therefore $\Delta M_A^{(\text{self})}(\text{"compute } \mu_B(E)\text{"}) = 0$: the computation induces no change to $A$'s self-model. By Definition M.10.2, $\sigma_A = 0$ for this computation, and by Corollary M.10.3.1, the reflexive cost component is zero. The computational cost for $A$ may be proportional to $C_{agg}(B)$ but is SPAP-flat as a function of $\mu_B(E)$.
...
*Proof.* Apply Theorem M.10.5(ii) with $B = S$. $\square$
...
**Theorem M.10.8 (Predictability of Perspectival Response).** Let $A$ be a system with $C_{agg}(A) > C_{agg}(B)$, and let $E$ be a pattern with $\mu_B(E) > 1/\alpha_{SPAP}$. Then $A$ can predict, prior to $B$'s exposure to $E$: (i) a lower bound on the thermodynamic cost $B$ will incur (via Theorem M.10.5(i) and Theorem M.10.3); (ii) which self-model components $\Delta M_B^{(\text{self})}(E)$ will be engaged (via the externally computed decomposition, Equation M.17); (iii) the cost concentration across self-model parameters (via the Fisher metric on $B$'s model space, computed externally).

*Proof.* (i) By Theorem M.10.5(i), $A$ can compute $\mu_B(E)$. By Theorem M.10.3, this determines a lower bound on processing cost as a deterministic function of $\mu_B(E)$. (ii) $A$ models $\mathcal{M}_B$ externally; the decomposition (Equation M.17) is deterministic given $\mathcal{M}_B$ and $E$. (iii) The cost contribution of parameter $\theta_i$ scales as $\|\delta\theta_i\|^2 / g(\delta_S)^2$; since $A$ can compute the Fisher metric on $B$'s model space, $A$ can identify cost-concentrating parameters. $\square$
...
- When $\sigma_S(E) > 0$, the transition engages $S$'s self-model. The cost acquires a SPAP-divergent component that is not captured by the geometric structure of $\Sigma$ alone — it depends on the self-referential depth of the transition, which is invisible to the perspective metric $d_\Sigma$ but quantified by $\mu_S(E)$.
...
| Theorem M.10.8 | Predictability from above | Thm M.10.5, M.10.3 |
...
**Cost Functional (M.6.10).** The perspectival profile $\mathcal{P}_S(E) = (\Delta Q_S, \mu_S, \sigma_S)$ provides the informational-thermodynamic cost functional on $\Sigma$ that the geometric formalism (M.2–M.5) did not supply. The SPAP proximity $\mu_S(E)$ measures how deeply a pattern engages the self-referential structure of a predictive system, and determines a content-dependent processing cost (Theorem M.10.3) that is physical (Theorem M.10.7) and asymmetrically measurable (Theorem M.10.5). Shannon information is recovered as the special case $\sigma_S = 0$ where the reflexive cost vanishes and measurement is symmetric (Corollary M.10.3.1). The perspectival profile is irreducible to all existing information-theoretic quantities (Theorem M.10.9).

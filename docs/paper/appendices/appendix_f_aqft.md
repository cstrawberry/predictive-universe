...
2.  **CC Influence Mediated by Global State $\omega$:** The CC mechanism (Hypothesis 3) proposes that a high-CC system $S_A$ in region $\mathcal{O}_A$ influences the probabilistic outcomes of 'Evolve' processes, which are fundamentally defined relative to the underlying global state of the MPU network (or its emergent AQFT counterpart, the state $\omega$). A specific local context $C_A$ (related to the internal state of $S_A$) leads the system to evolve into or occupy a specific global physical state $\omega_{C_A}$. This state $\omega_{C_A}$ on the global algebra $\mathfrak{A}$ may possess non-local correlations, including entanglement (Proposition 10).

...
A potential tension arises: the PU framework is fundamentally time-asymmetric due to the irreversible 'Evolve' process (Definition 27) with cost $\varepsilon_{\mathrm{phys}}\ge\varepsilon_0=\ln2$ (Theorem 31), yet CPT is a symmetry of the emergent theory. This is resolved by distinguishing two levels:
...
*Proof.* Part (1) follows directly from Theorem F.4. Part (2) follows from the thermodynamic ratchet (Appendix O, Theorem O.3): every 'Evolve' step increases entropy by at least $\varepsilon_{\mathrm{phys}}\ge\varepsilon_0=\ln2$. The CPT-conjugate of an entropy-increasing sequence is an entropy-decreasing sequence, which violates the Second Law. ∎

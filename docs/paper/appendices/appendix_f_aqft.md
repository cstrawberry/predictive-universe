...
3.  **Controlled Coarse-Graining:** The coarse-graining maps $\Phi_b$ (with scale $b \to 0$) converge to a continuum evolution in the diamond norm. Specifically, there exist constants $C, \gamma > 0$ such that:
    ...
    $$
    This convergence must hold uniformly on local algebras supported in balls of a fixed physical radius. It is an additional bridge hypothesis: bounded degree, fixed interaction radius, and locality of the Hamiltonian terms are necessary background conditions, but they do not by themselves prove the displayed diamond-norm estimate. In particular, on a fixed physical region the coherent Hamiltonian contribution scales with the number of local supports; under the natural extensive normalization of local terms one generically expects an $O(1)$ coherent part rather than $o(1)$. A theorem-level derivation of the displayed bound therefore requires either extra suppression/cancellation of the coherent part or a continuum formulation that retains a nonvanishing Hamiltonian limit.
...
Establishing the full convergence from discrete MPU dynamics to a stable, local AQFT satisfying the Haag-Kastler axioms remains a major open bridge. Beyond the regularity-necessity layer of Appendix C and the compactness inputs of Appendix D, the present manuscript does not yet supply: (i) a theorem fixing the local isotropy input at the one-shell level or an equivalent replacement strong enough for the continuum bridge, (ii) a uniform discrete curvature-dimension / Bochner estimate that transfers the rescaled family into the non-collapsed synthetic-Ricci class used in Section 11.4, (iii) the Euclidean rigidity input needed to identify the measured limit with $\mathbb{R}^4$ on the relevant branch, or (iv) a controlled coarse-graining theorem on local algebras that absorbs the coherent Hamiltonian part. Rigorously deriving the emergent dynamics generator $\mathcal{L}^*$ and validating these remaining bridge hypotheses of Theorem F.0 are therefore key future theoretical tasks for the PU framework.

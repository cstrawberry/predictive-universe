...
    \quad \text{(A.0.2)}
    $$
    (This measures the worst-case, input-state trace-distance error. It is the induced \$1!\to!1\$ channel distance and is bounded above by the diamond distance \$\delta\_\diamond := \tfrac12|\mathcal{E}*{\mathrm{actual}}-\mathcal{U}*{\mathrm{ideal}}|*\diamond\$, so \$p*{\mathrm{err},0} \le \delta\_\diamond\$. In experimental practice one often reports the average gate infidelity \$r = 1 - F\_{\mathrm{avg}}(\mathcal{E}*{\mathrm{actual}}, \mathcal{U}*{\mathrm{ideal}})\$; for CPTP maps on a \$d\$-dimensional system there are standard inequalities of the form \$c\_1(d), r \le \delta\_\diamond \le c\_2(d),\sqrt{r}\$, implying (for small errors) \$p\_{\mathrm{err},0} \lesssim \delta\_\diamond = \mathcal{O}(\sqrt{r})\$.)

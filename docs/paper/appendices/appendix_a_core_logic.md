...
*Proof.* For each $j\le N^*(S)$, the diagonal component $S_{\mathrm{diag}}^{(j)}$ is constructible by Lemma A.1.0 applied to the corresponding retained self-model prediction register, together with the independent-register amplification construction of Theorem M.10.4. The register-capacity condition $n_S\ge N^*(S)$ supplies the Fisher-orthogonal addressable self-model registers required by Theorem M.10.4.

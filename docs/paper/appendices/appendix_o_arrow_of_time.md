...
**Remark O.4.2 (Locus of Irreversibility).** The standard account of the arrow of time locates irreversibility in information loss: entropy increases, information scrambles across inaccessible degrees of freedom, and practical recovery becomes impossible. Theorem O.4 reveals a deeper mechanism. Grant the most favorable scenario: a system $A$ with $C_{agg}(A) > C_{agg}(B)$ reconstructs the complete prior state of $B$ and communicates it to $B$. By Theorem O.4(i), representing and evaluating such a candidate restoration description is sender-side SPAP-flat for $A$. But the communicated pattern $E_{rev}$ targets $B$'s own self-model at a depth that, for exact restoration, reaches the $\mu_B = \infty$ boundary (Theorem O.4(ii)). Even if $B$ processes a sub-exact approximation, that processing is itself an 'Evolve' cycle producing $\Sigma_B^{(\text{proc})} \ge \varepsilon \ge \ln 2$ (Theorem O.4(iii)), advancing $B$'s history rather than inverting it. The arrow does not depend on whether the information has been lost. It persists because the act of integrating self-referential content is itself irreversible.

**Remark O.4.3 (Hierarchy Opacity and In-Universe Reconstruction Limits).** The measurement asymmetry (Theorem M.10.5) induces a strict hierarchy of temporal access. Every level of the observer hierarchy (§P.5.8.3) can look downward: a system $A$ with $C_{agg}(A) > C_{agg}(B)$ can treat $B$'s self-model $\mathcal{M}_B$ as an external modeling target, represent a candidate restoration description of $B$'s prior self-state, and evaluate the associated burden of communication at sender-side SPAP-flat cost (Theorem O.4(i); Theorem M.10.5(i)). But every level is opaque to itself: no universal procedure within $A$ correctly computes $\mu_A(E)$ for all patterns $E$ with $\sigma_A(E) > 0$ (Theorem M.10.5(ii)), and exact restoration of $A$'s own prior self-model is unavailable from within $A$'s own perspective (Corollary O.4.3(iii); Theorems M.10.4 and M.10.6). No external vantage point outside the universe is required or invoked; the result holds for systems within a single closed universe governed by Hypothesis 1.

Consider the strongest possible in-universe modeler — a system $A$ with $C_{agg}(A) \gg C_{agg}(B)$ for any chosen target $B$. Such a system can still treat $B$ as externally modelable data (Theorem O.4(i); Theorem M.10.5(i)). What it never acquires is an Archimedean vantage over the universe or over its own complete self-model: $A$ remains a subsystem of the same global entangled state $\omega$ (Remark L.12.6), and the self-referential limits of Theorem M.10.5(ii) together with Theorems M.10.4 and M.10.6 still apply to $A$ itself. The decisive obstruction to temporal reversal therefore remains receiver-side: even granting the most favorable external description of $B$'s prior self-state, $B$ cannot integrate an exact specification of that state without encountering the obstruction of Theorem O.4(ii)–(iii).

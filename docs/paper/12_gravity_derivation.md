...
*   (A3) **Geometric Regularity and operational-continuum closure:** The MPU network lies on the $M=24$, $D=4$ operational-continuum branch packaged by Theorem 43.5. Theorem 43 supplies the regularity-necessity layer; Lemma C.6d and Theorem C.6e supply $D_4$ shell isotropy and noncollapse; Theorem C.6c and Theorem D.6e supply the noncollapsed RCD/Mosco–Cheeger closure under their curvature-transfer, finite-core, and recovery hypotheses; and Theorem 44a supplies the regular-branch manifold structure needed for Theorems 44–46 under its quantitative Euclidean-rigidity input.

...
Let $(M,g)$ be the emergent Lorentzian manifold of Section 11. At any point $p\in M$, consider a local causal horizon $\mathcal H_p$ with null generator $k^\mu$ affinely parameterized by $\lambda$ and Unruh temperature $T=\kappa/2\pi$. Assume:
(a) **Area law** $\delta S=\eta\,\delta A$ for horizon entropy (Appendix E), with $\eta=1/(4G)$.
(b) **Energy flux** $\delta Q=\int T_{\mu\nu}^{(MPU)}\,\chi^\mu d\Sigma^\nu$, with $\chi^\mu\approx \kappa \lambda k^\mu$ the approximate boost Killing field (to first order in $\lambda$, with $\kappa$ the surface gravity).
(c) **Raychaudhuri** with $\theta(0)=\sigma(0)=0$: $\frac{d\theta}{d\lambda}=-R_{\mu\nu}k^\mu k^\nu$ at $\lambda=0$.
*Units:* We work in natural units $k_B=\hbar=c=1$ here; $c$ is restored in Eq. (76).

If the **Clausius relation** holds for all such $k^\mu$: $\delta Q=T\,\delta S$, then
...
(Using $\eta=1/(4G)$ in $\delta S=\eta\,\delta A$ yields the coefficient $1/(8\pi G)$.)
...
\tag{75}
$$
Taking the divergence and using $\nabla^\mu T_{\mu\nu}=0$ and the contracted Bianchi identity gives $f(x)=\tfrac12 R+\Lambda$, which yields the Einstein equation stated. ∎

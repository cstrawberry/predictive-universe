...
For the branch-prefactor ledger used in the gravitational effective-action calculation, $\mathfrak GY_U^{(4)}$ records
$$
...
5. Let
$$
\mathcal K_{\mathrm{null},n}
:=
\bigcap_{\rho\in\mathcal R_n}
\ker D\widetilde\rho(q_{\mathrm{grav},n}^*)
\subset E_n
$$
be the response-null tangent subspace, where the certificate specifies every smooth envelope extension $\widetilde\rho$. Fix an auxiliary inner product on $E_n$ as part of the certificate and define
$$
\mathcal T_{\mathrm{grav},n}
:=
\mathcal K_{\mathrm{null},n}^{\perp}.
$$
Thus no nonzero direction in $\mathcal T_{\mathrm{grav},n}$ is null for every retained smooth response extension.
...
8. $\tau_{\mathrm{PCE},n}>0$ is the finite fluctuation scale fixed by the same accepted certificate. In Landauer-normalized units it may be set to $1$ only when the certificate explicitly states that normalization; on a LAN estimation subbranch it may instead be fixed by the sample normalization, as in Corollary 12.1e.0a. A KMS temperature fixes the modular equilibrium scale of the state, but it does not by itself identify this PCE fluctuation scale unless the certificate also identifies the PCE Hessian, modular generator, and noise normalization in the same units.
...
**Theorem 12.2 (Scale-Dependent Statistical and Thermodynamic Structures).** *On the branch statuses listed in the table, PU supplies the following scale-dependent structures or candidates; the microscopic row is a modular representation and not by itself thermodynamic equilibration:*
...
**Part C (Cosmological).** The de Sitter horizon at $r_\Lambda=\sqrt{3/\Lambda}$ has Gibbons--Hawking temperature $T_{dS}=(\hbar c/2\pi k_B)\sqrt{\Lambda/3}$ [Gibbons & Hawking 1977]. On a registered semiclassical false-vacuum branch, a Euclidean tunneling weight may contain an action factor $e^{-S_{\mathrm{inst}}}$; it does not determine vacuum energy by itself. Appendix U's five-mode value is a reference convention. Theorem U.13b proves only four-dimensional nullity of its sampled-angular Hessian; $\mathfrak C_{U,\mathrm{mark}}$, the exact exponent-calibration certificate $\mathfrak C_{U,\mathrm{act}}$ of Proposition U.14, $\mathfrak F_U^{(4)}$, and $\mathfrak R_\Lambda^{(4)}$ remain the successive index, action, weight, and physical-realization gates. ∎

**Remark 12.2.1: Conditional Shared Ledger.** On Appendix E's density, reset-support, saturation, and calibration branch, the ND--RID finite-transfer ledger enters the operational coupling defined by Equation E.9:
$$
G_{\mathrm{op}}=\frac{\eta\delta^2c^3}{4\hbar\chi C_{\max}}.
$$
Identifying $G_{\mathrm{op}}$ with measured $G$ requires an external calibration; the bare finite-transfer structure does not determine either coefficient.
Here $C_{\max}$ is supplied quantitatively by the completed reset-support branch of Proposition E.2a, with $C_{\max}^*=2\ln2$ only on the minimal $d_0=8$ residual-budget and saturation branch; refresh/minorization separately gives the strict inequality $C_{\max}<\ln d_0$ by Lemma E.1 and Theorem E.2. The Born, reset-support, refresh-capacity, and gravitational-calibration branches have distinct antecedents and can be intersected only after their bridge records are supplied. In particular, the Born rule is not derived from this capacity bound. It descends separately from the predictive observable algebra, PPI/PCE response-null quotienting, retained finite-response additivity and noncontextuality, an accepted Definition-8.2b full-domain or finite informationally complete positive-reconstruction certificate, and the applicable Born selector on that certified domain. The common origin is SPAP finite-response closure: simultaneous ledgers yield the Born/noncommutative probability structure, while sequential update ledgers yield entropy and finite boundary capacity.
...
$$
If the matter fields $\Phi$ satisfy their Euler–Lagrange equations and are coupled through the boundary-sufficient metric $g_{\mu\nu}$ selected by Corollary 46e, equivalently no species-dependent response metric survives the PPI quotient on this branch, then
$$
\nabla_\mu T^{\mu\nu}=0.
$$
...
*Step 1 (Homogeneity).* Apply H 12.3a.2 to the reparameterization $\lambda(s)=\alpha s$, $\alpha>0$. Then $dx/ds=\alpha v$ and $d\lambda=\alpha\,ds$, so
$$
L_p(v)\,d\lambda=L_p(\alpha v)\,ds.
$$
Therefore
$$
...
**Definition 12.5.3k (Finite Einstein-Branch Closure Record).** A finite Einstein-branch closure record is a tuple
$$
\mathfrak C_{\mathrm{EH}}
=
(\mathfrak Z_{\mathrm{cont}},\mathfrak P_{\mathrm{AQFT}},\mathfrak C^{\mathrm{KMS}},\mathfrak A_{\partial},\mathfrak T_{\mathrm{MPU}},\mathfrak Q_{\mathrm{null}},\mathfrak L_{2},\mathfrak O_{\mathrm{EH}},\chi_{\mathrm{EH}})
\tag{76ag}
$$
with the following entries.
...
**Theorem 12.5.3k.1 (Finite Einstein-Branch Closure).** If $\mathfrak C_{\mathrm{EH}}$ is accepted and $\mathfrak Q_{\mathrm{null}}$ is the zero-slack ledger, then the retained emergent metric satisfies
$$
G_{\mu\nu}+\Lambda g_{\mu\nu}=8\pi G T_{\mu\nu}^{(\mathrm{MPU})}.
\tag{76ai}
$$
If $\mathfrak Q_{\mathrm{null}}$ supplies a conserved retained slack tensor $T_{\mu\nu}^{(\mathrm{rec})}$, then the retained equation is
$$
G_{\mu\nu}+\Lambda g_{\mu\nu}
=
8\pi G\left(T_{\mu\nu}^{(\mathrm{MPU})}-T_{\mu\nu}^{(\mathrm{rec})}\right).

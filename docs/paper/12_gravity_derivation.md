...
*Proof:* Follows steps 1-4 in Section 12.3.2, leading from the Clausius relation on horizons (Eq 72) via Raychaudhuri (Equation 73) and universality to the tensor equation $R_{\alpha\beta} - 8\pi G T_{\alpha\beta}^{(MPU)} = f(x) g_{\alpha\beta}$ (Equation 75). Consistency with covariant conservation of \$T\_{\mu\nu}^{(MPU)}\$ (**Theorem B.5, Appendix B**) and the Bianchi identity requires $f(x)$ to be a constant, identifiable with $-\Lambda$ (adjusting for standard GR conventions). Rearranging Equation (75) with $f(x)=-\Lambda$ and restoring factors of $c$ yields the EFE (Equation 76). QED

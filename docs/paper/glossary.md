...
*   **Strict-Certificate Registry:** (Appendix P, Definition P.14.1m; Algorithm P.14.1m.0; Theorems P.14.1m.1 and P.14.1m.2; Corollary P.14.1m.3; Remark P.14.1m.4) Finite-record realization of the Global Strict-Certificate Ledger. The schema $\mathfrak G_{\mathrm{PU}}=(\mathcal V,\mathcal E,\sigma,\rho,\beta,\theta,\gamma,\delta)$ records vertices, dependency edges, status labels, residual lists, residual-control records, closure gates, overlap audits, and forward-lock indicators. Residual-control records distinguish $\textsf{bound}$ entries from $\textsf{budget}$ entries so that comparison-budget scales are not promoted to theorem-level intervals.
...
*   **Current-Graph Non-Collapse Decomposition of the Discrete Backbone:** (Appendix R, Proposition R.3.5e.2; Theorem R.3.5e.3; Corollary R.3.5e.4; Remarks R.3.5e.5 and R.3.5e.6; Appendix Z, Definition Z.35d and Theorem Z.35e) Separation of the recurrent ledger $(K_0,d_0,\varepsilon_0,a,b,M,k,D)=(3,8,\ln2,2,6,24,12,4)$ into source roles $(\mathcal C_3,\mathcal C_{\mathrm{cap}},\mathcal C_{\mathrm{tan}},\mathcal C_{\mathrm{kis}})$ and downstream coherence invariants $(J_{\mathrm{top}},J_{\mathrm{Cl}},J_{\mathrm{ar}})$. The current ledger proves non-collapse of the source roles inside the present proof graph; it does not prove pairwise-disjoint leaf inputs and does not exclude a future common parent invariant.
...
*   **False-Vacuum Fredholm Prefactor Certificate:** (Appendix U, Definition U.15d; Theorem U.15e; Corollary U.15f) Certificate machinery specifying the seven explicit factors ($\mathcal J_{\mathrm{coll}}$, $|\mathcal A_{\mathrm{neg}}|$, $\mathcal A_{\mathrm{ghost}}$, $\mathcal A_{\mathrm{fv}}$, $\mathcal A_{\mathrm{meas}}$, $\det'_\zeta\mathcal H_{\mathrm{false}}$, $|\det''_\zeta\mathcal H_{\mathrm{bounce}}|$) needed for a theorem-level forward $A_{\mathrm{eff}}^{\mathrm{Fred}}$ on the Appendix U bounce branch. Theorem U.15e gates the audit on an accepted certificate; performing the audit is a separate calculation. Without the certificate, the Appendix U working value remains a forward convention.

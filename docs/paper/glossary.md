...
*   **$\delta_{SPAP}$ (Delta-SPAP):** (Appendix B.3, Theorem B.2) The statistical gap $\delta_{SPAP}:=\alpha_{SPAP}-PP$ between the achieved predictive performance and the fundamental self-prediction limit. It appears in the certified lower bound $C_{\mathrm{uni}}=\Omega(\log(1/\delta_{SPAP})/\delta_{SPAP}^2)$; transfer to another operational complexity requires its stated domination bridge. The symbol is distinct from the lower Space-of-Becoming parameter $\alpha$.
...
*   **Law of Prediction:** (Theorem 19, Equation 22) On the continuous operational-complexity interpolation for a declared task and target scale, assume the normalized residual gap is continuous and composes multiplicatively under independent relative-budget stages. Then $g(x+y)=g(x)g(y)$ gives $g(x)=e^{-\kappa_{\mathrm{eff}}x}$ and
$$
...
*   **Logical Indeterminacy:** (Definition 12) The class-level limitation on universal exact prediction produced by the SPAP/RUD diagonal constructions for systems satisfying the required Property-R and closure hypotheses. It does not imply eventwise stochasticity. On a separately supplied stochastic complex-Hilbert branch, Theorem 28a represents normalized noncontextual finite-additive effect weights by the Born trace rule.
...
*   **Mosco-Bochner AQFT Bridge Certificate:** (Appendix F, Definition F.0c; Theorem F.0d) Certificate machinery combining Mosco convergence of finite propagation-cost Dirichlet forms, radius-2 Bochner control, noncollapse, local generator convergence, algebraic inductive-limit compatibility, Lieb-Robinson lightcone normalization, and positive-energy spectral control to produce the stable local AQFT envelope.
...
*   **Perspective-Indexed Semantic Closure:** (Appendix P.16b.2–P.16b.3) A family $R=(R_p)_{p\in P}$ of perspective-indexed judgment relations closed under base kernels, reflexivity, same-perspective transitivity, and guarded cross-perspective adoption. The least closure exists by Theorem P.16b.2.4, the labeled proof system is sound and complete by Theorem P.16b.2.7, and finite support prevents paradox-shaped cycles from deriving arbitrary non-reflexive judgments outside the supported domain by Theorem P.16b.3.2.

*   **Pfaffian Real-Orientation Certificate:** (Appendix K, Definition K.6.10a; Theorem K.6.10b) Certificate machinery orienting the real doubled quark Pfaffian line on a positive chamber. It forces $\arg\det M_q=0$ unless the quark determinant gap closes or an explicit topological overhead is paid.
...
\qquad \delta_{SPAP}:=\alpha_{SPAP}-PP,
$$
...
*   **Proposition 5 (Schrödinger Equation):** (Section 8.1) Continuous time-translation-symmetric transition-probability-preserving ray dynamics has a strongly continuous unitary implementation and hence a self-adjoint generator.

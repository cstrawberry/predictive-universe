...
*   **Mode-Channel Correspondence:** (Appendix Z, Theorem Z.10) The PCE equilibrium condition requiring internal QFI modes to match external spatial channels: $M_{\rm int} = M_{\rm phys}$. At thermodynamic equilibrium (Postulate 4; Theorem Z.9), $M_{\rm phys} = K(D)$ (kissing number). Combined with $M_{\rm int} = 2ab = 24$, Theorem Z.11 then selects $D = 4$ at the PCE-optimal equilibrium.

...
*   **Finite-Resource Profile of a Hidden-Variable Completion:** (Definition 14.4b.1; Theorem 14.4c) Finite record $(\mathcal Q_h^{\mathrm{fd}},\mathcal Q_h^{\mathrm{fr}},\bar{\mathcal Q}_h^{\mathrm{diag}},\mathrm{ns})$ classifying a hidden-variable completion by finite depth, finite resource, pre-commit communication closure, and sealed-core no-signaling. Theorem 14.4c separates SPAP-excluded self-access, no-signaling-exposed query access, local deviating model layers, and response-null inaccessible or post-commit completions.

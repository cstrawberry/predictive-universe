...
*   **EFE (Einstein Field Equations):** (Section 12, Theorem 50) On the operational Lorentzian continuum branch carrying the local Rindler/KMS certificate, saturated horizon entropy density, conserved Appendix-B source, Raychaudhuri and null-saturation hypotheses, and the four-dimensional local metric-action/Lovelock–Wald closure, the metric response obeys $R_{\mu\nu}-\tfrac12Rg_{\mu\nu}+\Lambda g_{\mu\nu}=(8\pi G/c^4)T_{\mu\nu}^{(MPU)}$ (Equation 76). Clausius is one antecedent of this conditional chain.
...
*   **Landauer-Noether Closure Principle:** (Appendix Q, Theorem Q.0.7d2; Corollary Q.0.7d3) Let a topological finite-response configuration space carry a continuous left $U(1)$ action, and let its continuous predictive response functional be invariant under the dense subgroup $G_L=\{e^{iN\ln2}:N\in\mathbb Z\}$. Continuity then extends the invariance to all of $U(1)$. A local Noether current follows only on the additional finite-resolution branch whose leading continuum limit has the minimally coupled matter--gauge form and whose independent cell phases act as the corresponding local redundancy.
...
*   **Module Decomposition Theorem:** (Theorem G.8.4b, Appendix G) In the finite-response positive-marginal block-frame capacity regime of Equation (G.8.5), with determinant-compatible anomaly-admissibility and response-null exact global phases quotiented, the unique capacity-saturating direct-sum decomposition of the inactive fiber $\mathcal B=\mathbb C^6$ is
    $$
    \mathcal B=\mathbb C^3\oplus\mathbb C^2\oplus\mathbb C^1.
    $$
    The derivation uses exhaustive block-partition analysis under: (i) the Lagrangian capacity bound $n_G \leq 12$ (Theorem G.8.2e); (ii) the positive-marginal saturation rule selecting admissible block-frame algebras with $n_G = 12$ whenever attainable; (iii) faithfulness; and (iv) determinant-compatible anomaly/hypercharge constraints. Under these conditions, the Standard Model gauge algebra
    $$
...
*   **Neural Scaling Law (PU form):** (Corollary 19.2, Equation 22h) The conditional image of the Law of Prediction under a logarithmic size-to-complexity relation $C-C_{op}=\eta_p\ln(N_p/N_{p,0})$: $\beta-PP(N_p)=(\beta-\alpha)(N_p/N_{p,0})^{-s_p}$ with $s_p=\kappa_{\mathrm{eff}}\eta_p/\hat C_{\mathrm{target}}$.
...
*   **Proposition 6 (Interpretation of Superposition):** (Section 8.2) On the complex Hilbert representation, vector expansions are interpreted as available predictive branches; the algebraic expansion itself does not derive physical actualization.

*   **Proposition 7 (Born Rule):** (Section 8.3, Theorem G.1.7) Normalized noncontextual weights additive on finite effect decompositions have trace form on the complex Hilbert branch of dimension at least three.
...
*   **SPAP (Self-Referential Paradox of Accurate Prediction):** (**Section 4.2, Theorems 10, 11, A.1.1, A.1.3**) Theorems demonstrating the logical impossibility of guaranteed perfect self-prediction (deterministic or probabilistic) for systems possessing the retained finite-program subcapacity needed for diagonalization: representation, simulation, predicate evaluation, logical memory, Boolean post-processing, and self-coding closure as formalized by §A.0.2 (Theorem A.0.1; Corollary A.0.1), with the diagonal constructions carried out in Theorems A.1.1 and A.1.3. Establishes fundamental Logical Indeterminacy and performance limit $\alpha_{SPAP}$.
...
*   **Vacuum–Electroweak Complexity Product Lock:** (Appendix T, Corollary T.62c; Appendix U, Corollaries U.72c–U.72d) On the Appendix U five-mode reference branch and Appendix T electroweak determinant branch,
$$
...
*   **Vacuum Weight Interpretation of $\Lambda$:** (Remark U.15c.1) $\Lambda L_P^2=8\pi A_{\mathrm{eff}}e^{-2\kappa}$ is the gravitational-unit expression of a residual vacuum-sector finite-action weight, with $\kappa=142$ theorem-level on the four-mode false-vacuum branch (Theorem U.13b) and $\kappa_{\Lambda,\mathrm{ref}}=141.5$ a five-mode reference convention.

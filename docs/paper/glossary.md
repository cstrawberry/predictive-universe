...
*   **Certificate Remainder:** (Appendix Z, Definition Z.27.11a) A bounded term in a numerical sector whose existence, sign, and allowed interval follow from a specified strict PPI/PCE certificate. It is not an adjustable theoretical tolerance. A sector containing a certificate remainder is theorem-closed only when the remainder is either evaluated exactly or bounded sharply by an accepted residual certificate of the form $\mathfrak R_\alpha^{\mathrm{AO}}$ (Definition Z.27.11g), $\mathfrak F_U$ (Definition U.15d), or analogous sector-specific finite records.

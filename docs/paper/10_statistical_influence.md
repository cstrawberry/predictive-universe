...
QCP is a pre-agreed decision protocol built from context-dependent statistical correlations. On branch (iii), if Alice's independently chosen context changes Bob's pre-lightcone marginal, the protocol defines a noisy channel with positive capacity and therefore violates operational no-signaling by Theorem 39c. Theorems 40–42 limit its rate, and Theorem 39a gives zero finite-window zero-error capacity on its registered branch; neither result makes the channel noncommunicating. On the marginal-invariant or shared-past branches, the procedure changes only joint or conditional statistics and creates no pre-lightcone message channel.
...
In summary, two causal versions of the model preserve the remote outcome distribution or attribute it to a shared earlier cause. A freely chosen late setting that changes a remote pre-lightcone outcome distribution would instead violate ordinary no-signaling.

**Technical ledger.**

The AQFT analysis (Appendix F) provides a conditional consistency setting for emergent locality on the exact marginal-invariant and shared-past branches; a late-randomized marginal anomaly is an external falsifier by Theorem 39c. The local CPTP branch preserves Bob's marginal exactly; the stronger branch-(iii) marginal anomaly is kept as the genuine statistical-FTL hypothesis and is constrained on the regular finite-window branch by Theorems 39a–42 rather than erased by ordinary no-signaling.

...
5. *Exclusion $d_0<8$.* **Lemma (No three commuting qubit roles in $d_0<8$).** Let $\mathcal A_1,\mathcal A_2,\mathcal A_3\subset B(\mathcal H)$ be $*$‑subalgebras, each $*$‑isomorphic to $M_2(\mathbb C)$, pairwise commuting and acting non‑trivially. Then the C$^*$‑algebra they generate is $*$‑isomorphic to $M_2(\mathbb C)\otimes M_2(\mathbb C)\otimes M_2(\mathbb C)\cong M_8(\mathbb C)$; in particular, any non‑degenerate representation requires $\dim\mathcal H\ge 8$. *Proof.* By the universal property of tensor products and the double‑commutant theorem, commuting matrix subalgebras embed as the spatial tensor product, which acts irreducibly on $\mathbb C^{2\cdot2\cdot2}$. Therefore no MPU with $d_0<8$ can realize the three roles while preserving SPAP closure and RID bounds. ∎

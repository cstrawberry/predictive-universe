...
The minimal Hilbert-space dimension $d_0=8$, established as a lower bound by the operational requirements of Theorem 15, is a unique and stable outcome of the framework's functional requirements. In any MPU that implements a full SPAP cycle with one irreversible RID of cost $\varepsilon\ge\ln 2$ (Appendix J) and satisfies the PU axioms (Sec. 5–8), the finite‑dimensional predictive C*-algebra $\mathfrak A$ must be capable of hosting three functionally distinct, binary roles. The necessity of simultaneously and non-destructively encoding these roles imposes a strict, non-negotiable lower bound on the dimension of the MPU's Hilbert space.

The three essential functional roles, each corresponding to a minimal binary degree of freedom, are:

1.  **The Quantum Register:** This role is necessary to hold the MPU's state in a superposition of potential outcomes. It provides the substrate for the probabilistic nature of prediction and is the minimal non-trivial quantum system ($M_2(\mathbb{C})$) to which the Gleason-type argument for the Born rule applies (as extended by Busch to POVMs for the $d=2$ case, see **Appendix G.1**).
2.  **The Pointer/Commit Register:** This role is required to hold the definite outcome of a predictive or interactive step, which is then subject to the irreversible 'Evolve' process. This register is the locus of the Landauer-limited thermodynamic cost ($\varepsilon \ge \ln 2$), as it must internally represent the bit of information that is logically erased to close the predictive loop (**Appendix J**).
3.  **The Recursion/Control Register:** This role is essential for managing the sequential phases of the Fundamental Predictive Loop. It acts as a control bit that gates the distinct 'Internal Prediction' and 'Evolve' phases, ensuring the logical integrity and non-destructive execution of the self-referential update at the core of SPAP (as required by conditions O1-O3 in **Theorem 15**).

For the MPU to function as an autonomous unit, these roles must be managed internally. The Principle of Compression Efficiency (PCE) selects for the most resource-efficient and robust implementation of the predictive cycle. While an MPU could theoretically time-multiplex its internal registers by synchronizing to an external control signal, this strategy incurs significant costs and risks disfavored by PCE. It introduces a perpetual communication cost (`V_prop`) to maintain the link, creates a dependency on an external source that may be noisy or unreliable, and complicates error correction. In contrast, internalizing the control logic into a dedicated register minimizes external dependencies, enhances robustness, and localizes the computational loop. For a *minimal* predictive unit, PCE therefore strongly favors the autonomous, closed-loop architecture, making the internalization of all three functional roles a derived, optimal strategy rather than an arbitrary assumption.

**Lemma (Minimal Hilbert Space for the Predictive Algebra).**
Let the predictive C*-algebra $\mathfrak{A}$ of an MPU be the minimal algebra capable of instantiating the three functionally distinct and dynamically interacting binary roles required by the Fundamental Predictive Loop (Quantum, Pointer, and Recursion registers), under the PCE-derived requirement for a closed-loop, autonomous architecture. At the commit snapshot of the cycle, these roles must be simultaneously and non-destructively readable, requiring their corresponding subalgebras to be pairwise commuting. Let these be $\mathfrak{A}_Q \cong M_2(\mathbb{C})$, $\mathfrak{A}_P$ (Pointer), and $\mathfrak{A}_R$ (Recursion), where $\mathfrak{A}_P$ and $\mathfrak{A}_R$ are isomorphic to either $\mathbb{C}^2$ (classical bit) or $M_2(\mathbb{C})$ (qubit). Then any non-degenerate, faithful representation of $\mathfrak{A}$ requires a Hilbert space $\mathcal{H}$ of dimension $\dim\mathcal{H} \ge 8$.

*Proof.* The requirement for simultaneous, non-destructive readability at the commit snapshot implies that the subalgebras $\mathfrak{A}_Q, \mathfrak{A}_P, \mathfrak{A}_R$ must commute at that instant. The C*-algebra generated by these three commuting subalgebras must contain a subalgebra isomorphic to $M_2(\mathbb{C}) \otimes \mathbb{C}^2 \otimes \mathbb{C}^2$. By the spectral theorem for commuting operators, the classical registers $\mathfrak{A}_P$ and $\mathfrak{A}_R$ are generated by four joint minimal projections $E_{jk}$ satisfying $\sum_{j,k \in \{0,1\}} E_{jk} = I$. The double-commutant theorem implies that the quantum register algebra $\mathfrak{A}_Q$ must act as a full $M_2(\mathbb{C})$ on the subspace projected by each $E_{jk}$. For a faithful representation, each of these four subspaces must be non-trivial, hence $\dim(E_{jk}\mathcal{H}) \ge 2$. The total dimension is therefore bounded by $\dim\mathcal{H} = \sum_{j,k} \dim(E_{jk}\mathcal{H}) \ge 4 \times 2 = 8$. If the pointer and recursion registers are themselves quantum ($\cong M_2(\mathbb{C})$), the generated algebra contains $M_2(\mathbb{C})^{\otimes 3} \cong M_8(\mathbb{C})$, which also requires $\dim\mathcal{H} \ge 8$. Any dimension $d_0 < 8$ is informationally insufficient to non-destructively manage the three required binary degrees of freedom.

It is critical to note that this requirement for commutation applies specifically to the storage registers at the 'commit snapshot' of the cycle, where information must be stably held and read. Between these moments of stability, the MPU's internal dynamics apply entangling unitary operations (e.g., CNOTs, as modeled in Sec. 7.1.3) that do not commute with the storage observables, thereby executing the necessary information processing and transfer between the functional roles. ∎

This establishes `d₀=8` as a strict lower bound. The Principle of Compression Efficiency ensures it is also the unique, stable dimension.

**Corollary (PCE Stability of `d₀=8`).**
Let $J(d) = \text{PP}(d) - \lambda \cdot \text{Cost}(d)$ be the PCE objective function, where the cost of maintaining the Hilbert space, $\text{Cost}(d)$, is a monotone and strictly convex function of the dimension $d$. Given that the SPAP constraints enforce a minimal dimension of 8, any local maximizer $d^*$ of the PCE objective must satisfy $d^*=8$.

*Proof.* The functional requirements of the predictive loop impose a hard constraint $d \ge 8$. For any dimension $d > 8$, the additional degrees of freedom beyond the minimal eight required for the core predictive algebra either (i) decouple from the predictive cycle, in which case they contribute zero marginal predictive performance ($\partial\text{PP}/\partial d = 0$) but a positive marginal cost ($\partial\text{Cost}/\partial d > 0$), making the configuration suboptimal, or (ii) couple to the cycle, incurring additional operational and error-correction costs. In either case, the strict convexity of the cost function ensures that any dimension greater than the minimum required represents an inefficient allocation of resources. The PCE dynamics, which seek to minimize net cost (or maximize $J(d)$), will therefore drive the system to shed any superfluous dimensions, making $d_0=8$ the unique, stable, and minimal dimension. ∎

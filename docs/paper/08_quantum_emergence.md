...
*Proof:* The derivation relies on the consistency of probability assignments within the emergent Hilbert space $\mathcal{H}_0$ (Proposition 4), yielding the probability law first proposed by Born [Born 1926]. For $\dim(\mathcal{H}_0) \ge 3$ (satisfied by $d_0 \ge 8$, Theorem 23), Gleason's theorem mathematically establishes that any probability measure $\mu(P)$ on projectors $P$ satisfying non-contextual additivity must uniquely take the form $\mu(P) = \text{Tr}(\rho P)$ for some density operator $\rho$. Crucially, the non-contextuality premise is not assumed; as rigorously shown in Appendix G.1 (Lemma G.1.1), non-contextuality is derived—not assumed—from the stability requirement of the PCE Potential $V(x)$. This is because a contextual cost function would render the potential $V(x,\mathcal{P})$ dependent on the arbitrary choice of measurement basis $\mathcal{P}$, making its gradient ill-defined and precluding the stable convergence of the system’s adaptation dynamics (proven in Appendix D). This optimization requirement, driven by POP (Axiom 1) and PCE (Definition 15), dynamically enforces the non-contextual additivity required by Gleason by demanding that the system’s internal cost accounting for potential outcomes be consistent and efficient. Therefore, the Born rule probability assignment $P(i | S(t), s) = \text{Tr}(|\psi\rangle\langle\psi| \cdot |i\rangle_s\langle i|_s) = |\langle i | \psi \rangle_s|^2$ (Equation 50) emerges not only as mathematically necessary for consistency within the Hilbert space, but also as the unique measure reflecting the optimal allocation of predictive resources determined by the framework's foundational optimization principles. QED

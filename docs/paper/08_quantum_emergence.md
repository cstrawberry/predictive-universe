...
Assume that the predictive weights form a normalized, non-contextual, additive functional on complete resolutions of the identity and are continuous under directed refinements induced by the PCE-optimal dynamics (Appendix G.1; Theorem 8.6). Under these conditions, and given that the MPU Hilbert space dimension satisfies $d_0 = 8 \geq 3$ (Theorem 23), the probability $P(i | S(t), s)$ for the 'Evolve' process (Definition 27), acting on an MPU with state amplitude $S(t)=|\psi\rangle$ relative to perspective $s$ (corresponding to ONB $\{|i\rangle_s\}$), is uniquely fixed to the Born rule by Gleason's theorem:
...
*Proof.* If $[\hat{A},\hat{B}]=0$, then (by the spectral theorem for commuting self-adjoint operators) $\hat{A}$ and $\hat{B}$ are simultaneously diagonalizable and admit a common eigenbasis. In a common eigenstate $|\psi\rangle$ one has $\Delta A=\Delta B=0$, contradicting complementarity. Hence $[\hat{A},\hat{B}]\ne 0$.

For the uncertainty relation, set $\hat{A}_0=\hat{A}-\langle\hat{A}\rangle I$ and $\hat{B}_0=\hat{B}-\langle\hat{B}\rangle I$ and define $|u\rangle=\hat{A}_0|\psi\rangle$, $|v\rangle=\hat{B}_0|\psi\rangle$. By Cauchy–Schwarz,
$$
\langle u|u\rangle\,\langle v|v\rangle \ge |\langle u|v\rangle|^2.
$$
The left-hand side equals $(\Delta A)^2(\Delta B)^2$. The right-hand side is $|\langle\psi|\hat{A}_0\hat{B}_0|\psi\rangle|^2$. Writing
$$
\hat{A}_0\hat{B}_0=\frac{1}{2}\{\hat{A}_0,\hat{B}_0\}+\frac{1}{2}[\hat{A}_0,\hat{B}_0],
$$
and using that $\{\hat{A}_0,\hat{B}_0\}$ is self-adjoint while $[\hat{A}_0,\hat{B}_0]$ is anti-self-adjoint, one obtains
$$
|\langle\psi|\hat{A}_0\hat{B}_0|\psi\rangle|^2 \ge \frac{1}{4}\,|\langle\psi|[\hat{A},\hat{B}]|\psi\rangle|^2.
$$
Taking square roots yields Robertson's bound:
$$

...
**Lemma 8.6a (PPI/PCE Isometry of the Reset-Free Internal Branch).**
On the Hilbert/Born branch, the reset-free and evidence-free Internal Prediction phase of Definition 26 is PPI/PCE-admissible only as an isometry of ray space with respect to the Fubini-Study metric. Equivalently, it preserves Born transition probabilities between rays.

*Proof.* During Internal Prediction no verification outcome is received and no SPAP reset is executed. Therefore the phase has no admitted finite-response input capable of creating new distinguishability and no admitted irreversible merge capable of erasing response-relevant distinguishability without paying the 'Evolve' reset ledger.

Let $\mathcal T$ be the deterministic ray map for one internal step. If $\mathcal T$ identifies two distinct response-relevant rays, then two different future Evolve-response presheaves are merged before verification. This is an irreversible response-relevant erasure outside the reset step, contradicting the reset-free definition of the internal branch. If the distinction is response-null, PPI quotients it before the state-space count, so it is not a distinct physical ray. Hence $\mathcal T$ is injective on retained rays.

If $\mathcal T$ increases the Fubini-Study distinguishability between two retained rays without an admitted finite-response input, it creates response-relevant distinguishability from no evidence. Such surplus distinguishability either changes no future protocol response and is removed by PPI, or changes future protocol response without a corresponding input. In the second case the dynamics is admitting an unsupported response generator that is not registered in the finite-response input ledger of Definition 26, which is exactly the surplus structure excluded by the PCE no-surplus clause of Definition 15: any response-relevant generator without a corresponding input cost is dominated by the smaller-cost competitor that omits it. PCE rejects both cases. If $\mathcal T$ decreases Fubini-Study distinguishability between retained rays, it erases response-relevant predictive information without the SPAP reset ledger, which is excluded by the reset-free branch condition. Therefore the retained map preserves Fubini-Study distance.

For rays $[\psi],[\phi]$,
$$
d_{FS}([\psi],[\phi])=\arccos|\langle\psi|\phi\rangle|.
$$
Preservation of $d_{FS}$ is therefore equivalent to preservation of
$$
|\langle\psi|\phi\rangle|^2,
$$
which is the Born transition probability between the rays. ∎

**Proposition 11 (Derivation of Schrödinger Equation for Internal Evolution)**

On the PPI/PCE-isometric internal branch of Lemma 8.6a, assume the Internal Prediction phase defines a continuous time-translation-symmetric ray dynamics on $\mathcal H_0$. Then the time evolution of the MPU state amplitude $S(t) \in \mathcal{H}_0$ is governed by the time-dependent Schrödinger equation.

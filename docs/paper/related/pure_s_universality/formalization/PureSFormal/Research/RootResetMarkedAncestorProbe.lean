...
/-! A fixed finite upward search for the nearest completed marked Local.
It includes the starting cursor, returns the exact first matching ancestor,
and ends at the root when no ancestor matches. -/
namespace PureSFormal.Research.RootResetMarkedAncestorProbe
...
inductive First (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) : Cursor → Bool → Cursor → Prop where
  | found (origin : Cursor) (accepted : RootResetCompletedLocalPatterns.accepts .marked program tree origin.focus = true) :
      First program tree origin true origin
  | root (source : Term) (refused : RootResetCompletedLocalPatterns.accepts .marked program tree source = false) :
      First program tree ⟨source, []⟩ false ⟨source, []⟩
  | parent (source : Term) (frame : ParentFrame) (parents : List ParentFrame)
      (refused : RootResetCompletedLocalPatterns.accepts .marked program tree source = false)
...
theorem First.found_parsed {program : CTS.Program} {tree : Dispatcher.Tree (ActionLabel program)}
    ...
    ∃ view : CheckpointDecoder.LocalView program, view.status = .marked ∧
      CheckpointDecoder.parseLocal? program tree endpoint.focus = some view := by
  generalize foundEq : true = found at first
  induction first with
  | found origin accepted => exact (RootResetCompletedLocalPatterns.accepts_iff_parse .marked program tree origin.focus).mp accepted
...
end PureSFormal.Research.RootResetMarkedAncestorProbe

...
namespace PureSFormal.Research.RootResetParametricFirstEmptyStages
...
theorem completeMixedNonfinalAt
    ...
    (selectedJobs : JobSelection (selector := selector) program dispatcher (bit :: suffix) (previous + remaining + 1))
    ...
    ∀ (count : Nat) (countBound : count ≤ previous + remaining + 1) (sampleIndex : Nat) (outerParents : List ParentFrame)
      ...
      SelectedMixedInvariantAt (selector := selector) program dispatcher bit suffix previous remaining count
...
theorem firstEmptyRawAt
    ...
    (selectedJobs : JobSelection (selector := selector) program dispatcher (bit :: suffix) (fuel + 1))
  ...
  obtain ⟨terminalParents, nonfinalConfigurations, nonfinalChain,
    ...
    completeMixedNonfinalAt laws program dispatcher bit suffix previous
...
end PureSFormal.Research.RootResetParametricFirstEmptyStages

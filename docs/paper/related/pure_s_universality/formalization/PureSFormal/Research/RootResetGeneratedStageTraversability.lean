...
/-!
# Generated stage parents retain the selector's traversal premises

All three stage constructions carry the exact raw scheduler segment together
with clean completed parents for the next clock stage, for every finite input.
-/

namespace PureSFormal.Research.RootResetGeneratedStageTraversability

open PureSFormal.PureS
open SchedulerControl SchedulerInvariant SchedulerCycle
  SchedulerResponseInvariant SchedulerCompletedContext SchedulerStageAssembly
  SchedulerNestedResponse SchedulerNestedPhase SchedulerRecurrence SchedulerTraceAlgebra
...
theorem allNonemptyRawAt
  ...
  obtain ⟨terminalParents, nonfinalConfigurations, nonfinalChain,
    ...
    RootResetGeneratedJobTraversability.completeNonfinalAt program dispatcher bit suffix fuel
  ...
  have checkpointIndex : sampleIndex + configurations.length =
    ...
    rw [completeLength]
    exact indexEq
  refine ⟨nextParents, configurations, checkpoint, semantic, ?_, layerEq ▸ terminalClean⟩
  refine
    { chain := ?_
...
theorem firstEmptyRawAt
  ...
  obtain ⟨terminalParents, nonfinalConfigurations, nonfinalChain,
      nonfinalSampled, nonfinalLength, nonfinalOuter⟩ :=
    RootResetGeneratedEmptyTraversability.completeMixedNonfinalAt program dispatcher bit suffix previous
...
theorem emptyRawAt
  ...
  obtain ⟨terminalParents, nonfinalConfigurations, nonfinalChain,
      nonfinalSampled, nonfinalLength, nonfinalOuter⟩ :=
    RootResetGeneratedEmptyTraversability.completeEmptyNonfinalAt program dispatcher fuel fuel
      (sampleIndex + phaseConfigurations.length) parents
      (CheckpointRun.cumulativeLayers fuel) outer
  ...
  obtain ⟨terminalPrefix, terminalChain, terminalPrefixSampled,
    ...
    SchedulerNestedEmpty.terminalEmptyJobRawSegment program dispatcher
      ...
      (CheckpointRun.cumulativeLayers fuel + fuel) nonfinalOuter.toCompleted
  ...
  let rawShapes :=
    SchedulerNestedEmpty.terminalEmptyRawShapesAt program dispatcher [] fuel
      finalRegisters finalCarrier terminalParents
      (CheckpointRun.cumulativeLayers fuel + fuel) nonfinalOuter.toCompleted finalDecode
      phaseEq
  ...
  have completeLast : LastSample configurations checkpoint := by
    simpa [configurations, terminalConfigurations, checkpoint, finalRegisters,
      ...
      (LastSample.appendLeft
        (phaseConfigurations ++ nonfinalConfigurations) terminalLast)
  refine ⟨_, configurations, checkpoint, semantic, ?_, layerEq ▸ nonfinalOuter.empty finalRegisters [] finalCarrier⟩
  refine
    { chain := completeChain
      count := completeLength
      last := completeLast
...
end PureSFormal.Research.RootResetGeneratedStageTraversability

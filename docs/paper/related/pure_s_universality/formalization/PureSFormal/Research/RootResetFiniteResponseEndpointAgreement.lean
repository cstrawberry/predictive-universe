...
namespace PureSFormal.Research.RootResetFiniteResponseEndpointAgreement
...
def Selected (program : CTS.Program) (layout : ActionDispatcher program)
  ...
  ∃ endpoint after, RootResetEdgeFragment.follow address
      ⟨localTerm, parentsAfter (compileActions program layout.tree) layers parents⟩ = some endpoint ∧
    endpoint.rdx? = some after ∧
    RootResetFinitePrioritySelector.selectStep? program layout
      (Cursor.rebuild parents (wrap (compileActions program layout.tree) layers localTerm)) = some after.erase

theorem accepts_none {program : CTS.Program} {tree : Dispatcher.Tree (ActionLabel program)}
    (status : CheckpointDecoder.HaltStatus) {source : Term}
    (none : CheckpointDecoder.parseLocal? program tree source = Option.none) :
  ...
  | false => rfl
  | true =>
      obtain ⟨view, kind, parsed⟩ := (RootResetCompletedLocalPatterns.accepts_iff_parse status program tree source).mp accepted
      rw [none] at parsed
      cases parsed
...
theorem fresh_selected {program : CTS.Program} {layout : ActionDispatcher program}
  ...
  have appendedNone : ∀ audit, CheckpointDecoder.parseLocal? program layout.tree
      (.app (CheckpointDecoder.openShell (freshHField carrier) dispatcher seed seedAudit continuation continuationAudit) audit) = none := by
    intro audit
    apply CheckpointDecoder.parseLocal?_none_of_headArity
    · change 7 ≠ 5; decide
    · change 7 ≠ 6; decide
  have freshClear : RootResetFreshHistoryRejection.Clear program layout.tree
      (CheckpointDecoder.openShell (freshHField carrier) dispatcher seed seedAudit continuation continuationAudit) :=
    ⟨accepts_none .fresh noLocal, fun audit => accepts_none .fresh (appendedNone audit)⟩
  have markedClear : RootResetHistoricalPriorityRejection.Clear program layout.tree
      (CheckpointDecoder.openShell (freshHField carrier) dispatcher seed seedAudit continuation continuationAudit) :=
    ⟨accepts_none .marked noLocal, fun audit => accepts_none .marked (appendedNone audit)⟩
  cases contracted : endpoint.rdx? with
  | none => rw [contracted] at redex; cases redex
  | some after =>
      exact ⟨endpoint, after, followed, contracted,
        RootResetHistoricalPriorityRejection.endpoint_selected outer layers _ freshClear markedClear
          frontTicks false frontRun used (.done true) endpoint after execution rfl contracted⟩
...
end PureSFormal.Research.RootResetFiniteResponseEndpointAgreement

...
/-!
# Traversability preserved by the actual nonempty job recurrence

The exact scheduler job executors are strengthened to retain the generated
fresh-layer decoder fact. Their mutation lists, costs, sampled-state proofs,
and literal next parents are certified together with traversal provenance.
-/

namespace PureSFormal.Research.RootResetGeneratedJobTraversability
...
/-- An actual selected response with a nonempty successor contributes a
traversable fresh layer; its literal decoder fact comes from targetDecode. -/
theorem selectedResponseTrace_fresh
    ...
    CleanParents program dispatcher
      (SchedulerRootContinuation.freshContinuationParents program dispatcher
        ...
        (deletedCarrier bit outerContext innerContext) parents) (layers + 1) := by
  apply outer.selectedResponseTrace_fresh trace
  rw [dataEq]
  exact List.cons_ne_nil nextBit nextSuffix
      ...
      | cons nextBit nextSuffix =>
          ...
          obtain ⟨configurations, chain, sampled, lengthEq, nextOuter⟩ :=
            selectedFreshNonterminalJobAt program dispatcher inputBits seedBits
              ...
              sampleIndex outerParents layers outer.toCompleted trace' nextBit nextSuffix
              dataEqRegisters
          let nextParents :=
          ...
          · dsimp [nextParents, finalRegisters, carrier]
            exact selectedResponseTrace_fresh outer trace' dataEqRegisters
      ...
      | cons nextBit nextSuffix =>
          ...
          obtain ⟨firstConfigurations, firstChain, firstSampled, firstLength⟩ :=
            selectedPendingSegmentAt program dispatcher inputBits seedBits
              ...
              (Nat.succ_ne_zero remaining) outerParents layers outer.toCompleted
...
/-! ## All nonfinal jobs preserve clean completed parents -/

def NonfinalInvariantAt
    (program : CTS.Program) (dispatcher : ActionDispatcher program)
...
theorem completeNonfinalAt
    ...
    ∀ (count sampleIndex : Nat) (outerParents : List ParentFrame)
      (layers : Nat)
      (outer : CleanParents program dispatcher outerParents layers),
      NonfinalInvariantAt program dispatcher bit suffix fuel count sampleIndex
  ...
  | count + 1, sampleIndex, outerParents, layers, outer => by
      ...
      obtain ⟨responseParents, checkRegisters, responseConfigurations,
        ...
        completeNonemptyJobAt program dispatcher bit
      ...
      have tail := completeNonfinalAt program dispatcher bit suffix fuel allNonempty count
...
/-! ## The terminal job preserves the exact next-stage parent invariant -/
  ...
  | succ remaining =>
      ...
      obtain ⟨terminalRegisters, terminalPhase, terminalBit,
        ...
        selectedNonemptyPendingPrefixAt program dispatcher bits bits continuation
          (Dovetail.clockExit_admissible (remaining + 1) 0 environment)
          outerParents layers outer.toCompleted remaining sampleIndex
          (Registers.newJob program).clearScan (CTS.zeroPhase program) bit suffix
      ...
      | cons nextBit nextSuffix =>
          ...
          have semantic : FreshTerminalData program dispatcher bits
            ...
            simpa [bits, environment, continuation, finalCarrier, nextParents,
              ...
              (selectedFreshTerminalShapeAt program dispatcher bits remaining
                ...
                terminalSuffix outerParents layers outer.toCompleted
          ...
          · dsimp [nextParents, finalCarrier]
            exact selectedResponseTrace_fresh outer terminalTrace dataEq
...
end PureSFormal.Research.RootResetGeneratedJobTraversability

...
/-! A finite clock-core probe with exact nested-origin failure return.
The wrapper spine uses only finite pattern probes and R descent; failure
reverses exactly those edges. Its enclosing input cursor is never stored. -/

namespace PureSFormal.Research.RootResetNestedClockCoreProbe
...
theorem all_input (origin : Cursor) (boundary : rightParent? origin = false) :
  ...
  | some row =>
      ...
      obtain ⟨target, subterm⟩ := heads_subterm row member descended.focus matched
...
theorem all_input_with_walk (origin : Cursor) (boundary : rightParent? origin = false) :
    ∃ ticks ready endpoint, ticks ≤ coefficient * origin.focus.size ∧
      run machine ticks (initial origin) = ⟨some (.done ready), endpoint⟩ ∧
      (if ready then endpoint.rdx?.isSome = true ∧ RootResetEdgeSpine.Walks pendingRows origin endpoint else endpoint = origin) := by
  obtain ⟨downTicks, descended, noMember, downBound, downRun, downWalk⟩ :=
    RootResetEdgeSpine.scan_within pendingRows pending_valid origin
  obtain ⟨downUsed, downUsedBound, actualDown⟩ := descending_runs origin descended downTicks noMember downRun
  cases selected : RootResetEdgeFragment.select headRows descended.focus with
  ...
  | some row =>
      ...
      subst row
      have same : descended = endpoint := Option.some.inj followed
      subst endpoint
      obtain ⟨readMember, readRun⟩ := RootResetEdgeFragment.selected_runs headRows descended descended headRow selected followed
      obtain ⟨readUsed, readUsedBound, actualRead⟩ := reading_runs descended descended _ true readMember readRun
      simp only [↓reduceIte] at actualRead
      refine ⟨downUsed + 1 + (readUsed + 1), true, descended, ?_, ?_, ⟨redex, downWalk⟩⟩
      · exact Nat.le_trans (Nat.le_add_right _ 1)
          (combined_bound origin.focus downUsed readUsed 0 (Nat.le_trans downUsedBound downBound)
            (Nat.le_trans readUsedBound (RootResetEdgeFragment.ticks_bound _ _)) (Nat.zero_le _))
      · rw [run_add, actualDown]
...
end PureSFormal.Research.RootResetNestedClockCoreProbe

...
namespace PureSFormal.Research.RootResetMixedNonemptyStages
...
theorem completeNonfinalAt
    ...
    ∀ (count : Nat) (countBound : count ≤ fuel + 1) (sampleIndex : Nat) (outerParents : List ParentFrame)
      ...
      SelectedNonfinalInvariantAt program dispatcher bit suffix fuel count sampleIndex
  ...
  | count + 1, countBound, sampleIndex, outerParents, layers, outer => by
      ...
      have responseSelected := RootResetMixedNonemptySweep.completeJob_selectorChain
        program dispatcher bit suffix (fuel + 1) (count + 1) (fuel + 1)
        countBound outerParents layers outer (Nat.succ_ne_zero fuel) allNonempty
...
theorem allNonemptyRawAt
  ...
  obtain ⟨terminalParents, nonfinalConfigurations, nonfinalChain,
    ...
    completeNonfinalAt program dispatcher bit suffix fuel
  ...
  have terminalSelected := RootResetMixedNonemptySweep.completeJob_selectorChain
    ...
    (Nat.succ_ne_zero fuel) allNonempty terminalChain terminalLength
...
end PureSFormal.Research.RootResetMixedNonemptyStages

...
/-!
Locate the nearest completed marked response before deciding between response work
...
-/
namespace PureSFormal.Research.RootResetMarkedCandidateProbe
...
def ancestorAnswer? {program : CTS.Program} {tree : Dispatcher.Tree (ActionLabel program)} :
    RootResetMarkedAncestorProbe.Control program tree → Option Bool
  | .done found => some found
  | _ => none

def ancestor (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) : Worker :=
  ⟨RootResetMarkedAncestorProbe.Control program tree, RootResetMarkedAncestorProbe.machine program tree,
    .probing ⟨RootResetMarkedAncestorProbe.classifier program tree, ProbeCompiler.Control.self_mem_nodes _⟩,
    ancestorAnswer?⟩
...
theorem ancestor_terminal (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) :
  ...
  intro state ready answered origin ticks
  cases state with
  | done result => exact RootResetMarkedAncestorProbe.done_absorbs program tree result origin ticks
...
def coefficient (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) : Nat :=
  RootResetActiveMarkedFrontend.coefficient program tree + RootResetMarkedAncestorProbe.coefficient program tree + 2
...
theorem readOnly (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) : (worker program tree).ReadOnly :=
  RootResetProbeSequence.readOnly _ _ (RootResetActiveMarkedFrontend.mutationCount_zero program tree)
    (RootResetMarkedAncestorProbe.mutationCount_zero program tree)
...
structure Result (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program))
  ...
  active : ∃ ready endpoint, RootResetActiveMarkedFrontend.Trace program tree (Cursor.atRoot source) ready endpoint ∧
    RootResetMarkedAncestorProbe.First program tree endpoint found candidate
...
theorem all_input (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program)) (source : Term) :
  ...
  obtain ⟨searchTicks, found, candidate, searchBound, searchRun, first⟩ := RootResetMarkedAncestorProbe.all_input program tree endpoint
  ...
  · simpa only [coefficient, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm, Cursor.atRoot, Cursor.erase, Cursor.rebuild]
      using combined_bound (Cursor.atRoot source) (RootResetActiveMarkedFrontend.coefficient program tree)
        (RootResetMarkedAncestorProbe.coefficient program tree) frontUsed searchUsed 2 frontPaid searchPaid (Nat.le_refl _)
  · change run (RootResetProbeSequence.machine _ _) _ (RootResetProbeSequence.initial _ _ _) = _
    rw [run_add, frontActual]
    exact searchActual
  · have unchanged := RootResetMarkedAncestorProbe.erase_run program tree searchTicks (RootResetMarkedAncestorProbe.initial program tree endpoint)
...
theorem generated (program : CTS.Program) (tree : Dispatcher.Tree (ActionLabel program))
    ...
    (searchRun : run (RootResetMarkedAncestorProbe.machine program tree) searchTicks
      (RootResetMarkedAncestorProbe.initial program tree endpoint) = ⟨some (.done found), candidate⟩) :
...
end PureSFormal.Research.RootResetMarkedCandidateProbe

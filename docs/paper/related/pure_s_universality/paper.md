...
## 1. Problem and central idea {#section-1}

The S Combinator Challenge asks whether the preassigned rule
$\mathbf SXYZ\to XZ(YZ)$ can perform arbitrary computation \[[1](#reference-1),[3](#reference-3)\].
...
and decoding of results during a continuing S computation \[[3](#reference-3)\], pp. 85--86.
...
is decidable for pure S \[[5](#reference-5),[42](#reference-42)\], so it cannot serve as the target halting
predicate of this computable reduction. The output is read from a completed
checkpoint on the continuing path.

The worked example in [Section 3](#section-3) exercises the controller on a small cyclic-tag
instance. The retained padded universal encoder produces at least $2^{57207}$
literal seed bits for every input; [Section 7.4](#section-7-4) derives this bound and states
the scope of the executions.


## 2. Model and main theorem {#section-2}

### 2.1 Terms and native contraction {#section-2-1}
...
A step contracts one occurrence at one selected address. [Appendix B](#appendix-b) identifies
the implementation and its agreement with this rule.

### 2.2 Finite selection from the current term {#section-2-2}
...
counter or auxiliary return stack. [Section 5](#section-5) gives the passes and proof.


### 2.3 Source instances and returned bits {#section-2-3}
...
row scanning $b$. [Section 6.1](#section-6-1) gives the exact initialization and proves
agreement with an independent conventional two-sided infinite tape.

### 2.4 Theorem 1R. Exact computation by a fixed root-restarted finite controller {#theorem-1r}
...
**Proof dependencies.** [Section 4](#section-4) establishes the checkpoint invariant with
a persistent scheduler as a proof device. [Section 5](#section-5) proves that the concrete
...
of [Section 5.4](#section-5-4). [Section 6](#section-6) composes the universal compiler
and literal readback equivalences; [Section 7](#section-7) gives the measured bounds.
...
component fields are linked below; [Appendix B](#appendix-b) and the
...
| Accompanying cumulative bounds in [Section 7.3](#section-7-3) | [Native-path bound](formalization/generated/public_theorem_signatures.md#cumulative-invocation-bound-on-every-native-s-path); [source-sampling bound](formalization/generated/public_theorem_signatures.md#cumulative-fresh-root-source-microticks) |


### 2.5 Four clocks {#section-2-5}
...
The checkpoint map $j=\tau_w(n)$ is strictly increasing. [Corollary 2.2](#corollary-2-2)
...
## 3. A worked computation {#section-3}

\label{guide-example}

### 3.1 Queue cells and cyclic-tag steps {#section-3-1}
...
![One cell contraction. The two occurrences of X are separate subtrees: the active predecessor is followed; the retained duplicate is history.](paper/figures/queue_cell.png)
...
### 3.2 Computing 101 → 011 → 11 {#section-3-2}
...
and never around a history copy. The scheduler recurrence in [Appendix B](#appendix-b)
...
these two arrows; the public recurrence is listed in [Appendix B](#appendix-b).

### 3.3 Observed checkpoints {#section-3-3}
...
Every intervening contraction sample is rejected by [Theorem 1](#theorem-1).
...
is reported in [Section 7.4](#section-7-4), p. \pageref{guide-execution}.

### 3.4 Two successive invocations from the root {#section-3-4}
...
has the required terminal continuation. [Section 5.2.2](#response-decision) gives the complete
decision table; [Appendix E](#appendix-e) gives the literal contractions in the $101$ run.

## 4. Representation and exact checkpoints {#section-4}

\label{guide-native}

### 4.1 The five fields {#section-4-1}
...
resulting queue. [Section 4.2](#section-4-2) gives the literal
pure-$\mathbf S$ constructors, [Section 3](#section-3) and [Appendix E](#appendix-e) trace the data movement and
one complete transition, and [Section 4.7](#section-4-7) states the invariant and exact cost
recurrences.

### 4.2 The checkpoint grammar {#section-4-2}
...
The remaining registered forms are summarized below; [Appendix B](#appendix-b) maps them to
...
| completed response | the fresh or marked shell in [Equation (6)](#equation-g5) |
...
inductive definitions. [Appendix B](#appendix-b) identifies the exact family-indexed Lean
predicates used by the scheduler invariant. [Equations (4)](#equation-g3)--[(6)](#equation-g5) give the
...
### 4.3 Literal checkpoint parsing {#section-4-3}
...
proved for every parser stage, and [Theorem 1](#theorem-1) proves that the composite accepts
...
### 4.4 Fresh bounded recomputation {#section-4-4}
...
### 4.5 The persistent scheduler used in the proof {#section-4-5}
...
address, stack, or unbounded integer belongs to the control value. [Section 4.7](#section-4-7)
...
control. [Section 7.2](#section-7-2) gives its exact resource boundary; [Appendix E.4](#appendix-e-4) gives
...
### 4.6 Theorem 1. Exact cyclic-tag trajectories under a fixed persistent-cursor evaluator {#theorem-1}
...
control state with its cursor at the root of $E_P(w)$. Thus [Equation (7)](#equation-1)
...
Taking $P$ to be the fixed 912-phase program $U$ of [Theorem 2](#theorem-2) fixes $M_U$,
$D_U$, and the detector once for all encoded deterministic tape instances.
[Equation (7)](#equation-1) is the exact computation trajectory underlying the halting
equivalence in [Equation (20)](#equation-2).

**Kernel correspondence.** [Appendix B.2](#formal-theorem-1) lists the checked declarations. The public ledger gives their complete types and exact axiom sets.

### 4.7 Exactness and progress {#section-4-7}
...
H[\mathbf S X(v_iX)].\tag{9}
$$

Under the canonical-front premise, the local queue-decoding relation assigns
$i::u$ to the active field before [Equation (9)](#equation-11) and $u$ afterward. This local
...
\longrightarrow N(JX)(X(JX)).\tag{10}
$$

The distinguished accumulator becomes $JX$; the second argument $X(JX)$ is
retained history. Iterating [Equation (10)](#equation-12) for an appendant
...
$2(h+1)+6$ contractions, after which the job theorem applies [Equation (9)](#equation-11),
the finite dispatcher route, [Equation (10)](#equation-12), and the marker rule exactly as
...
the literal queue $\delta_P^{h+1}(I_P(w))$; [Equation (12)](#equation-14)
...
of [Equation (7)](#equation-1), including rejection of every intervening contraction.

The whole-term transition-family lemma in [Section 5.2.5](#whole-term-transition-agreement), beginning on
...
[Appendix B](#appendix-b) identifies the checked declarations for the simultaneous sampled-state invariant and raw-microtick erasure theorem,
...
its finite appendant and summing [Equation (11)](#equation-13) yields
$\tau_w(n)\le C_P(n+1)^3$. One contraction at most doubles an unshared
occurrence tree, which yields [Equation (24)](#equation-4). These are bounds in separate
units, as detailed in [Section 7.3](#section-7-3).

## 5. Selection from a fresh root {#section-5}
...
| Pending handoff | After C4, the parity query returns false. Ascend to the pending parent and contract its root redex. | Even if C4 consumed the last cell, this response cannot COMMIT yet. The pending wrapper and parity invariant distinguish it from a completed empty response ([Section 5.2.2](#response-decision)). |
| Empty-origin response | Nonemptiness fails and the EMPTY-origin query succeeds. COMMIT contracts the fresh halt field at $LLL$. | An initial or repeated EMPTY has no ordinary preceding deletion. Its separate origin test prevents an incorrect parity-based handoff; COMMIT preserves every other field ([Section 5.2.2](#response-decision)). |
| Completion | An ordinary completed response with true parity and no live cell commits its halt field. A marked Local, or a fresh Local with nonempty result, admits its $RL$ continuation. | A historical fresh outer Local has a nonempty accumulator and is declined; marked outer layers are traversed. The completed-parent invariant is preserved ([Section 5.2.1](#active-copy-exclusion)). |
...
syntax. [Section 4.3](#section-4-3) derives the two continuations; [Section 7.4](#section-7-4) records their
execution.

### 5.1 The concrete finite passes {#section-5-1}

The root-restarted machine uses the local observations and primitive moves defined in
[Section 2.2](#section-2-2). It starts each invocation at the root in one fixed control state. Its
...
![Finite control flow of one root-restarted invocation. Each declined pass ascends to the root; each success performs one contraction.](paper/figures/root_reset_control.png)
...
term of size $N$. [Section 5.2.4](#section-5-2-4) gives the termination argument;
...
### 5.2 Recovery, the empty case, and termination {#section-5-2}

**Proof of [Theorem 1R](#theorem-1r) (selection).** Shell and Local name the completed
response constructor of [Equation (6)](#equation-g5). The constructor
...
![Active continuation and historical occurrences in a completed Local. Descent follows the solid arrows to K. The candidate search returns along parent edges; an old enclosing X is tested and declined, while the copy inside the audit is not a candidate.](paper/figures/root_history_roles.png)
...
#### 5.2.3 Exact recovery of the contraction path {#section-5-2-3}
...
the two trajectories at every contraction index, proving [Equation (1)](#equation-1r).
...
#### 5.2.4 All-input termination {#section-5-2-4}
...
[]{#equation-rootpotential}

$$
 t+1+\alpha_P|v|\leq\alpha_P|u|,\qquad |v|<|u|. \tag{13}
$$
...
$$
 ...
 \operatorname{microticks}(M)\leq K_P(|M|+1). \tag{14}
$$
...
completed-parent invariant proved in [Section 5.2](#section-5-2).
...
### 5.3 Proof dependencies {#section-5-3}



![Main simulation dependencies. The fixed endpoint uses the selected-path theorem; the confluence obstruction and certificate-enumeration theorem are independent of it. Theorem 1R transfers the selected trajectory to fresh-root finite control.](paper/figures/theorem_dependencies.png)

[Theorem 1](#theorem-1) follows from the scheduler invariant, the exact checkpoint
recurrence, and the exclusion theorem. [Theorem 2](#theorem-2) composes [Theorem 1](#theorem-1) with five
compiler equivalences. [Theorem 3](#theorem-3) refines [Theorem 1](#theorem-1) with resource bounds.
[Theorem 1R](#theorem-1r) reconstructs every contraction of [Theorem 1](#theorem-1) with the finite
root-restarted selector; source-output transfer then preserves [Corollary 2.1](#corollary-2-1).
Theorems 4 and 5 are independent of this construction.



### 5.4 A fixed regular halting language {#section-5-4}
...
priority selection passes of [Section 5.2](#section-5-2), stopping before contraction.
...
[Section 4.2](#section-4-2). The admitted continuation $B$ has head arity three or four,
...
This proves soundness and [Equation (15)](#equation-regularcts). No program-counter
...
the same period-912 program and dispatcher as [Theorem 1R](#theorem-1r), and let
$\mathcal L_{\mathrm{halt}}$ be the accepted tree language. [Equation (15)](#equation-regularcts) and the
padded compiler's halting equivalence give

[]{#equation-regularsource}
...
through the computable map $x\mapsto E(x)$ and [Equation (16)](#equation-regularsource),
...
## 6. Universal compilation and source output {#section-6}

\label{guide-compilation}

### 6.1 The conventional source machine {#section-6-1}
...
equivalence. [Appendix B](#appendix-b) lists the checked initialization, forward and backward
...
construction. Use Shannon's fixed-length block encoding \[[39](#reference-39)\], pp. 163--165:
...
### 6.2 The fixed compiler target {#section-6-2}
...
![The premise-free compiler chain from deterministic tape machines to the fixed 912-phase cyclic tag program and the scheduled pure-S trajectory.](paper/figures/simulation_pipeline.png)
...
[Theorem 2](#theorem-2) composes all five equivalences. [Appendix B](#appendix-b) gives the exact
...
The number 912 follows from Cook's constructions \[[30](#reference-30)\], Sections 1.2--1.3.
...
`6a25246ca4d3c04fce206376fd83c4336cce29cae226cdf9bc30b778adea0ccc`. The commands
...
114 productions and 912 appendants. [Appendix D](#appendix-d) gives the normalized table and
...
$\operatorname{Dec}_{\rm TM}$ defined in [Theorem 2](#theorem-2). Their composition is the
complete program $\Theta$ for the exact tape-instance encoder.

For [Theorem 2](#theorem-2), name the intermediate outputs of the checked compiler chain
...
appears in [Theorem 2](#theorem-2). The retained arithmetic route materializes unary counter
values and can grow doubly exponentially as an unshared representation.
[Section 7.3](#section-7-3) separates that size fact from the qualitative halting theorem.

### 6.4 Theorem 2. One fixed pure-S endpoint for deterministic Turing-machine halting {#theorem-2}
...
The fixed-program definition and period declaration are listed in [Appendix B](#appendix-b).
...
be the Boolean word produced by the compiler composite displayed in [Section 6](#section-6),
and set
...
The program, scheduler, decoder, and detector in [Equation (20)](#equation-2) are independent
of $x$. The transparent structural map $E_{\mathrm{TM}}$ and its exact
halting equivalence are identified in [Appendix B.3](#formal-theorem-2). [Equation (20)](#equation-2) is
...
**Kernel correspondence.** [Appendix B.3](#formal-theorem-2) lists the checked declarations. The public ledger gives their complete types and exact axiom sets.
...
[Equation (20)](#equation-2), this gives a computable many-one reduction from deterministic
...
**[Corollary 2.1](#corollary-2-1) (Literal source computation and output).** Let
...
**[Corollary 2.2](#corollary-2-2) (Ordered finite source prefixes).** If the source run is
defined through step $H$, there exists a clock $\theta_{x,H}:\mathbb N\to
\mathbb N$ with $\theta_{x,H}(0)=0$ and

[]{#equation-sourceorder}
...
observer are those of [Theorem 1R](#theorem-1r).
...
Finally, the native-S checkpoint theorem of [Section 4.7](#section-4-7) supplies one
strictly increasing contraction clock for the CTS iterates and rejects
every intervening contraction sample.

#### 6.5.1 Literal tape rows and primitive counter boundaries {#section-6-5-1}
...
#### 6.5.2 Counter words and the restricted deletion-two adapter {#section-6-5-2}
...
#### 6.5.3 Rogozhin's represented program and data regions {#section-6-5-3}

Use the normalized symbols $s_0,\ldots,s_5$ of [Appendix D](#appendix-d); the printed unary symbol is $s_0$, the data separator is $s_5$, and the blank is $s_4$. For a restricted table $\pi$, define
...
The program region $Q$ consists of the halting prefix $s_3s_1$, the production frames in descending label order, and a final separator $s_1$. A production frame records the reversed payload after its first two distinguished labels, followed by the distinguished weight and its difference from the current label's weight. [Appendix D.6](#app-d-6) gives its literal exponent formula, delimiters, and inverse;
[Appendix D.7](#app-d-7) proves the complete macro and strict-interior exclusions. These literal frames, rather than a source-machine interpreter, supply the inverse's weights and finite alphabet dimensions.
...
#### 6.5.4 Cook arrivals, phase alignment and cleanup {#section-6-5-4}

[Appendix D.8](#app-d-8) gives the complete production schema, the represented and
...
an additional radix encoding ([Appendix D.8](#app-d-8)).
...
#### 6.5.5 The observers are partial inverses of present syntax {#section-6-5-5}
...
#### 6.5.6 Composition and the exact order conclusion {#section-6-5-6}
...
## 7. Interface bounds and execution evidence {#section-7}

\label{guide-bounds}

### 7.1 Theorem 3. Exact observation and explicit structural bounds {#theorem-3}

Use the notation of [Theorem 1](#theorem-1). Let $\Lambda_P$ be the finite set of
...
[Appendix B](#appendix-b) identifies $\Lambda_P$, $r_P$, and $a_P$ with their formal
definitions.
...
[Equations (23)](#equation-3)--[(25)](#equation-5) measure different resources. [Equation (23)](#equation-3) counts strict
contextual contractions from the initial term to a checkpoint. [Equation (24)](#equation-4)
counts nodes in the completely unshared checkpoint term. [Equation (25)](#equation-5) counts
...
checkpoint decoder and marked detector, respectively. [Appendix B.11](#primitive-resource-derivation)
...
has a separate explicit quadratic coefficient. [Equations (27)](#equation-7)--[(28)](#equation-8) concern
...
The source-row, terminal-row, and scanned-bit observers of [Corollary 2.1](#corollary-2-1)
...
[Equation (29)](#equation-9) isolates the cyclic-tag word-to-term generator. [Theorem 2](#theorem-2)'s
complete encoder certificate composes it with the exact compiler $W$.

**Kernel correspondence.** [Appendix B.4](#formal-theorem-3) lists the checked declarations. The public ledger gives their complete types and exact axiom sets.

### 7.2 What the interfaces do {#section-7-2}

This section isolates the external interfaces from the coupled dynamics of
[Theorem 1](#theorem-1). “Evaluator alone” means that focused contraction is disabled: the
...
$\mathbf S$-contraction. Theorems 1--3 describe this persistent baseline. [Theorem 1R](#theorem-1r) proves that
...
contraction; both declarations are identified in [Appendix B](#appendix-b) and the trusted
...
that storage. [Theorem 1](#theorem-1) proves what the coupled controller and rewriting
system compute. [Appendix B](#appendix-b) gives the exact declarations for E1--E3. The
...
$d_P(|T|+1)^2$ primitive operations in the reference model of [Theorem 3](#theorem-3).
...
constant plus $18|w|$. [Theorem 2](#theorem-2) also supplies the closed program $\Theta$
...
complete encoder, including the state-padding variant of [Corollary 2.1](#corollary-2-1).
...
### 7.3 Separate resource units {#section-7-3}
...
No row equates one unit with another. [Appendix B](#appendix-b) lists the corresponding
declarations.
...
motivate the separate shared-arena model. [Theorem 3](#theorem-3) itself concerns ordinary
unshared terms.
...
and neither auxiliary bound is used to prove [Theorem 1](#theorem-1).
...
tag data. The two retained-compiler growth declarations in [Appendix B](#appendix-b) exhibit
...
The conditional source-horizon declaration in [Appendix B](#appendix-b) uses four natural
...
supplies the external motivation for such a premise \[[23](#reference-23),[24](#reference-24)\]; the improved
bound appears in \[[24](#reference-24)\], theorem 4.3.2. In the checked statement, $L$ and the
horizon inequality are explicit premises.

### 7.4 Scope of execution evidence {#section-7-4}
...
abstract mathematical proofs \[[1](#reference-1)\]. The unexecuted large demonstration is
...
## 8. Formal assurance and related work {#section-8}

\label{guide-related}

### 8.1 Formal statements and trusted definitions {#section-8-1}
...
recorded in [Appendix B](#appendix-b), which maps them to exact declaration names. The
architectural results of [Appendix F](#appendix-f) also have Lean proofs and public exports,
with their correspondence recorded in Appendices B and F. The complete 319-entry kernel-elaborated
...
development admits only `propext` and `Quot.sound`. [Appendix F](#appendix-f) additionally
uses standard classical logic where recorded by its exact axiom reports.
Of the 319 public exports, 228 depend on `Quot.sound` and seven [Appendix F](#appendix-f)
...
### 8.2 Related work {#section-8-2}

Wolfram states both the selected-path question and a stronger all-branches
question \[[1](#reference-1),[11](#reference-11)\]. Theorems 1--3 establish the former in the baseline evaluator
model of [Section 4.5](#section-4-5); [Theorem 1R](#theorem-1r) transfers that computation to the fresh-root finite
controller of [Section 5.1](#section-5-1). Theorems 4--5 concern the unrestricted graph and are
independent of the halting reduction.

#### 8.2.1 One-point bases and results specific to S {#section-8-2-1}

Fokker constructs a one-combinator basis by bracket abstraction, and Goldberg
gives a general schema for one-point bases \[[33](#reference-33),[34](#reference-34)\]. They construct alternative
...
constructions \[[38](#reference-38)\], pp. 2--11. Zachos proves
confluence, nonrecurrence, and that every normalizing ground $\mathbf S$
term is strongly normalizing \[[44](#reference-44)\], pp. 50--54. His seventh result also
...
[Section 5.4](#section-5-4) fixes this family as one regular tree language
...
Waldmann decides normalization of ground pure-$\mathbf S$ terms \[[5](#reference-5)\],
theorem 55, p. 13; the thesis procedure is \[[42](#reference-42)\], theorem 8.6.1, p. 79.
He also gives a regular grammar for the normalizing terms \[[5](#reference-5)\],
...
characterization \[[40](#reference-40)\], proposition 7.1 and example 7.10. Its minimal
...
expansion \[[40](#reference-40)\], Lemma 7.3. Our automaton instead recognizes the selected
halting event of [Section 5.4](#section-5-4).

Cheilaris, Ramirez, and Zachos give an explicit regular grammar with a
manual derivation of its correctness \[[46](#reference-46)\], pp. 1--5, 6--25. Recognition by
...
persistence of normal-form right subterms \[[46](#reference-46)\], pp. 14--15. The
...
infinitely often \[[42](#reference-42)\], Chapter 7, pp. 61--68. The root restart in
[Theorem 1R](#theorem-1r) begins a read-only search; the subsequent contraction occurs
...
\[[41](#reference-41)\], pp. 3:5, 3:9. Vatan's theorem 4.2 rules out a pure-$\mathbf S$
term $\Sigma$ satisfying $\Sigma x\to^*x$ for every term $x$ \[[4](#reference-4)\].
...
Mitschke and Mitschke give early pure-S nonnormalization results \[[47](#reference-47)\],
pp. 1--2. Zachos
gives explicit nonnormalization families and proofs for the two smallest
ground exceptions, which contain seven $S$ leaves \[[44](#reference-44)\], pp. 55--56,
...
order \[[45](#reference-45)\], pp. 31--41. His pattern-reachability relation allows the
reduct to contain the target as a subterm \[[45](#reference-45)\], p. 15. The surrounding
...
evolution at exact checkpoints \[[6](#reference-6),[21](#reference-21)\]. Iwami--Nakano extend that line to
$\mathbf S$-like systems \[[22](#reference-22)\]. *Dance of the Starlings* proves an infinite
head reduction for $SSS(SSS)(SSS)$ in its first theorem \[[10](#reference-10)\]. Thus an explicitly
...
#### 8.2.2 Rewrite strategies and evaluator state {#section-8-2-2}
...
\[[7](#reference-7)\], claim 4.1 and theorem 5.2, pp. 414, 417. His corollary 5.3, p. 417,
...
logic \[[43](#reference-43)\], sections 2 and 4, pp. 2--5 and 6--8. Effectiveness means total
...
choose a cofinal one-step reduction \[[43](#reference-43)\], sections 1--2, pp. 1--5. The
...
has no nonempty cycles \[[53](#reference-53)\], section 4, theorem 3. Douglas formalizes an
...
fixed representatives \[[52](#reference-52)\], `PathEncoding` and
...
between two supplied pure-$\mathbf S$ terms \[[52](#reference-52)\],
...
its arity-saturated application \[[48](#reference-48)\], pp. 346--347.
...
bases with positive reduction power \[[48](#reference-48)\], pp. 350--351. Native
...
[Section 7.3](#section-7-3) separately bounds accumulated selection work using the size
growth of the actual $\mathbf S$ path.

Zachos also describes leftmost, subterms-first and third-argument-first
strategies, using list, tree and shared representations \[[44](#reference-44)\], pp. 61--74.
...
under a global strategy \[[45](#reference-45)\], pp. 36--41. His residual-marking method
generalizes a proof by replacing unused subterms with variables
\[[45](#reference-45)\], pp. 46--47. His CLS implementation searches for patterns and
verifies restricted instances of three of the four schemas
\[[45](#reference-45)\], pp. 59--66; it uses explicit positions, integer pattern indices
...
and its path context \[[9](#reference-9)\].
...
control do not even recognize all regular tree languages \[[32](#reference-32)\]. A walker with
...
non-erasing variant \[[8](#reference-8)\]. Their machines inspect finite node labels and add
...
The model has a DLT 2023 precursor \[[51](#reference-51)\]; the theorem numbers and
non-erasing simulation cited here refer to the expanded 2026 article \[[8](#reference-8)\].
...
with \[[8](#reference-8)\] is the realization of queue, dispatcher, and checkpoint operations
...
Rosen's confluence theorem \[[2](#reference-2)\], theorem 5.6, p. 171, supplies the join
principle behind [Theorem 4](#theorem-4); the combinatory-logic application appears on
p. 177. Winskel's event structures supply a
standard comparison for occurrence histories and their causal prefixes \[[31](#reference-31)\];
they do not turn the observer-checked construction of [Theorem 5](#theorem-5) into a
target-local simulation.

#### 8.2.3 Universal-machine lineage and mechanized computability {#section-8-2-3}

The fixed endpoint combines Rogozhin's $(4,6)$ universal machine \[[25](#reference-25)\], Cook's
Turing-machine-to-tag and tag-to-cyclic-tag constructions \[[26](#reference-26),[30](#reference-30)\], and the
Cocke--Minsky and Minsky tag/counter lineage \[[27](#reference-27),[28](#reference-28)\]. Ibarra--Trân show that
deterministic two-counter programs with unary input initially in one counter
and no input tape cannot recognize all recursive sets \[[29](#reference-29)\]. [Section 6](#section-6) identifies the exact conventions
used in the checked specialization. Neary--Woods motivate the conditional
polynomial route isolated in [Section 7.3](#section-7-3) \[[23](#reference-23),[24](#reference-24)\].

Machine-checked computability theory predates this development. Carneiro
formalizes partial recursive functions in Lean \[[35](#reference-35)\]; Xu--Zhang--Urban
mechanize Turing machines and a universal-machine result in Isabelle/HOL \[[36](#reference-36)\];
Forster--Kunze--Wuttke verify Turing-machine programming in Coq \[[37](#reference-37)\]. The
pure-$\mathbf S$ normalization work also has machine-checked components
\[[40](#reference-40)\]. The use of a proof assistant is therefore not itself a novelty claim.
Turner and Danvy--Zerny supply combinator implementation and operational
semantics \[[12](#reference-12),[17](#reference-17)\]. Graph-to-term correspondence is studied in \[[13](#reference-13)--[15](#reference-15)\],
while \[[16](#reference-16),[18](#reference-18)\] establish cost invariance under specified strategies and
representations. Avanzini--Moser analyze complexity for acyclic term graphs
\[[20](#reference-20)\]. Asperti--Mairson analyze the nonelementary cost of implementing
Lévy-family parallel reduction \[[19](#reference-19)\]. These results provide context for the
separately premised shared-arena refinement.

#### 8.2.4 Direct comparison and priority scope {#section-8-2-4}

| Work | Prior result or model | Difference here |
|-----------------------|-----------------------------------|------------------------------------------|
| Fokker; Goldberg \[[33](#reference-33),[34](#reference-34)\] | constructed one-point bases | their basis term is not native $\mathbf S$ |
| Zachos \[[44](#reference-44)\] | native-S growth, nonnormalization families, supplied-target reachability and concrete strategies | arbitrary source events on the fixed selected path have no supplied target-size bound |
| Dörges \[[45](#reference-45)\] | inductive-pattern proofs of infinite native-S paths, subterm reachability and CLS analysis | exact whole-term checkpoints and one fixed finite local controller restarted at the root |
| Waldmann; Endrullis et al.; Vatan \[[4](#reference-4),[5](#reference-5),[40](#reference-40),[42](#reference-42)\] | pure-$\mathbf S$ normalization and extensional expressibility | selected trajectory observation is a different predicate |
| Cheilaris et al. \[[46](#reference-46)\] | regular normalization grammar with a manual derivation and linear-time recognition | normal-form existence at one term differs from eventual membership in the selected halting language |
| Dauchet \[[7](#reference-7)\] | a constructed simulation rule; specialization to a universal machine fixes one rule | the present rule is the preassigned native $\mathbf S$ rule |
| Endrullis--Zantema; Nakano--Iwami \[[6](#reference-6),[21](#reference-21),[22](#reference-22)\] | sole-combinator nontermination certificates | exact iterates are not decoded |
| Canal \[[48](#reference-48)\] | normalization costs for restricted bases and combinations | local work per selected contraction is bounded on every finite input, including nonnormalizing terms |
| Statman \[[43](#reference-43)\] | effective cofinal strategies using recursive graph searches | finite local control and linear all-term selection are additional requirements |
| Kutrib--Meyer \[[8](#reference-8),[51](#reference-51)\] | universal finite-control tree storage, including the non-erasing variant in \[[8](#reference-8)\] | the current term is the store, and only the native $\mathbf S$ contraction changes it |
| Klop et al.; Douglas \[[52](#reference-52),[53](#reference-53)\] | pure-S acyclicity and exclusion of injective path-preserving SK encodings | repeated source rows can be read from distinct target terms at different times |
| Carneiro; Xu et al.; Forster et al. \[[35](#reference-35)--[37](#reference-37)\] | mechanized computability and Turing machines | precedent for mechanization, not pure-$\mathbf S$ universality |
...
| contextual contraction ([`Step`](formalization/generated/public_theorem_signatures.md#contextual-pure-s-contraction)) | Replacement of one saturated $\mathbf SXYZ$ occurrence by $XZ(YZ)$ inside an arbitrary term context. |
...
| Shell / Local | The same completed response constructor; its continuation is at $RL$ ([Section 5.2](#section-5-2)). |
...
| certificate observer ([Appendix C](#appendix-c)) | A total current-term procedure that parses exposed certificate records and checks their adjacent source transitions. |
...
## Appendix B. Formal declaration map {#appendix-b}
...
from their counted clauses. It is part of the argument for [Section 7](#section-7).

The entries below are generated from the same pinned Lean `#check` output as
the complete 319-entry machine-readable ledger. They identify the declarations
used by Theorems 1R and 1--5 and [Appendix F](#appendix-f) without reproducing their full elaborated types inside
the narrative paper.
...
### B.1 Theorem 1R and the fixed root-reset aggregate {#formal-theorem-1r}

- **Complete root-reset headline contract.** [`PureSFormal.RootResetHeadline.HeadlineUniversality`](formalization/generated/public_theorem_signatures.md#complete-root-reset-headline-contract).
- **Complete root-reset headline theorem.** [`PureSFormal.RootResetHeadline.sCombinatorIsRootResetUniversal`](formalization/generated/public_theorem_signatures.md#complete-root-reset-headline-theorem).
- **Finite root-reset selection at every contraction.** [`PureSFormal.Research.RootResetFiniteAllInputsTraceAgreement.selectsEveryContractionRun`](formalization/generated/public_theorem_signatures.md#finite-root-reset-selection-at-every-contraction).
- **Uniform finite root-reset universality.** [`PureSFormal.Research.RootResetFiniteAllInputsTraceAgreement.finiteCTSUniversality`](formalization/generated/public_theorem_signatures.md#uniform-finite-root-reset-universality).
- **Bare-term iteration equals the certified path.** [`PureSFormal.Research.RootResetFiniteAllInputsTraceAgreement.termOnlyPath_eq_persistentPath`](formalization/generated/public_theorem_signatures.md#bare-term-iteration-equals-the-certified-path).
- **Contract projection equals actual runtime output.** [`PureSFormal.Research.RootResetContractProjection.projection_agrees`](formalization/generated/public_theorem_signatures.md#contract-projection-equals-actual-runtime-output).
- **Fixed root-reset computation contract.** [`PureSFormal.RootResetChallenge.RootResetComputationUniversality`](formalization/generated/public_theorem_signatures.md#fixed-root-reset-computation-contract).
- **Fixed root-reset computation universality theorem.** [`PureSFormal.RootResetChallenge.sCombinatorIsRootResetComputationUniversal`](formalization/generated/public_theorem_signatures.md#fixed-root-reset-computation-universality-theorem).
- **Root-reset returned scanned-bit output.** [`PureSFormal.Computation.DeterministicTapeRootResetOutput.exists_scannedOutput_iff`](formalization/generated/public_theorem_signatures.md#root-reset-returned-scanned-bit-output).
- **Measured root-reset output agreement.** [`PureSFormal.Computation.DeterministicTapeRootResetOutput.source_output_iff`](formalization/generated/public_theorem_signatures.md#measured-root-reset-output-agreement).
- **Ordered literal source sampling for every defined finite prefix.** [`PureSFormal.Computation.SourceChronologyRootReset.exists_sourceClock`](formalization/generated/public_theorem_signatures.md#ordered-literal-source-sampling-for-every-defined-finite-prefix).
- **Cumulative invocation bound on every native-S path.** [`PureSFormal.Research.RootResetCumulativeResources.path_cumulative_bound`](formalization/generated/public_theorem_signatures.md#cumulative-invocation-bound-on-every-native-s-path).
- **Cumulative fresh-root source microticks.** [`PureSFormal.Research.RootResetCumulativeResources.source_cumulative_bound`](formalization/generated/public_theorem_signatures.md#cumulative-fresh-root-source-microticks).

### B.2 Theorem 1 {#formal-theorem-1}

- **Uniform selected-path realization.** [`PureSFormal.WeakPathUniversality.finiteCTSWeakPathUniversality`](formalization/generated/public_theorem_signatures.md#uniform-selected-path-realization).
- **Selected-path exact recognition.** [`PureSFormal.WeakPath.Realizes.decode_eq_some_iff`](formalization/generated/public_theorem_signatures.md#selected-path-exact-recognition).
- **Selected-path noncheckpoint exclusion.** [`PureSFormal.WeakPath.Realizes.rejectsOther`](formalization/generated/public_theorem_signatures.md#selected-path-noncheckpoint-exclusion).

### B.3 Theorem 2 {#formal-theorem-2}

- **Fixed selected-path period-912 endpoint.** [`PureSFormal.WeakPathUniversality.fixedCookWeakPathUniversality`](formalization/generated/public_theorem_signatures.md#fixed-selected-path-period-912-endpoint).
- **Premise-free deterministic-tape to fixed-Cook endpoint.** [`PureSFormal.Computation.DeterministicTapeCook.halts_iff_fixedCookEmpty`](formalization/generated/public_theorem_signatures.md#premise-free-deterministic-tape-to-fixed-cook-endpoint).
- **Premise-free deterministic-tape selected-path endpoint.** [`PureSFormal.WeakPathUniversality.deterministicTapeHalts_iff_fixedPureSMarkedSnapshotTermEvent`](formalization/generated/public_theorem_signatures.md#premise-free-deterministic-tape-selected-path-endpoint).
- **Named pointwise deterministic-tape halting equivalence.** [`PureSFormal.WeakPathUniversality.fixedPureSMarkedSnapshotTapeEncoder_reducesVia`](formalization/generated/public_theorem_signatures.md#named-pointwise-deterministic-tape-halting-equivalence).
- **Complete tape encoder program agreement.** [`PureSFormal.Computation.CookEncodingComputability.Program.eval_sourceTerm`](formalization/generated/public_theorem_signatures.md#complete-tape-encoder-program-agreement).
- **Complete tape encoder is primitive recursive.** [`PureSFormal.Computation.CookEncodingComputability.sourceTerm_primitiveRecursive`](formalization/generated/public_theorem_signatures.md#complete-tape-encoder-is-primitive-recursive).
- **Effective deterministic-tape halting reduction.** [`PureSFormal.Computation.DeterministicTapeEffectiveReduction.effectiveReduction`](formalization/generated/public_theorem_signatures.md#effective-deterministic-tape-halting-reduction).
- **Output path starts at the certified encoder.** [`PureSFormal.Computation.DeterministicTapePureSOutput.termAt_zero`](formalization/generated/public_theorem_signatures.md#output-path-starts-at-the-certified-encoder).
- **Every source-output path edge is a native S contraction.** [`PureSFormal.Computation.DeterministicTapePureSOutput.termAt_step`](formalization/generated/public_theorem_signatures.md#every-source-output-path-edge-is-a-native-s-contraction).
- **Output-preserving encoder is primitive recursive.** [`PureSFormal.Computation.DeterministicTapePureSOutput.encode_code_primitiveRecursive`](formalization/generated/public_theorem_signatures.md#output-preserving-encoder-is-primitive-recursive).
- **Exact bare-term source-row readback.** [`PureSFormal.Computation.DeterministicTapePureSOutput.exists_literalRow_iff`](formalization/generated/public_theorem_signatures.md#exact-bare-term-source-row-readback).
- **Exact bare-term terminal-row readback.** [`PureSFormal.Computation.DeterministicTapePureSOutput.exists_terminalRow_iff`](formalization/generated/public_theorem_signatures.md#exact-bare-term-terminal-row-readback).
- **Exact scanned-bit output preservation.** [`PureSFormal.Computation.DeterministicTapePureSOutput.exists_scannedOutput_iff`](formalization/generated/public_theorem_signatures.md#exact-scanned-bit-output-preservation).
- **Nonhalting source runs reject terminal observations.** [`PureSFormal.Computation.DeterministicTapePureSOutput.nonhalting_rejects_terminal`](formalization/generated/public_theorem_signatures.md#nonhalting-source-runs-reject-terminal-observations).
- **Fixed Boolean negation through the pure-S path.** [`PureSFormal.Computation.DeterministicTapePureSOutput.bitToggle_output_iff`](formalization/generated/public_theorem_signatures.md#fixed-boolean-negation-through-the-pure-s-path).

### B.4 Theorem 3 {#formal-theorem-3}

- **Selected-path exact recognition.** [`PureSFormal.WeakPath.Realizes.decode_eq_some_iff`](formalization/generated/public_theorem_signatures.md#selected-path-exact-recognition).
- **Selected-path noncheckpoint exclusion.** [`PureSFormal.WeakPath.Realizes.rejectsOther`](formalization/generated/public_theorem_signatures.md#selected-path-noncheckpoint-exclusion).
- **Selected-path cubic checkpoint bound.** [`PureSFormal.WeakPathUniversality.finiteCTSCheckpointTime_le_explicit_cubic`](formalization/generated/public_theorem_signatures.md#selected-path-cubic-checkpoint-bound).
- **Selected-path checkpoint tree-size bound.** [`PureSFormal.WeakPathUniversality.finiteCTSCheckpointTerm_size_le_explicit`](formalization/generated/public_theorem_signatures.md#selected-path-checkpoint-tree-size-bound).
- **Selected-path per-contraction microtick bound.** [`PureSFormal.WeakPathUniversality.finiteCTSNextContractionMicroticks_le_linear`](formalization/generated/public_theorem_signatures.md#selected-path-per-contraction-microtick-bound).
- **Primitive public decoder certificate.** [`PureSFormal.PureS.PrimitiveInterfaceCertificates.primitivePublicDecoder_resource_certificate`](formalization/generated/public_theorem_signatures.md#primitive-public-decoder-certificate).
- **Primitive marked detector certificate.** [`PureSFormal.PureS.PrimitiveInterfaceCertificates.primitiveMarkedDetector_resource_certificate`](formalization/generated/public_theorem_signatures.md#primitive-marked-detector-certificate).
- **Primitive immutable-seed readback certificate.** [`PureSFormal.PureS.PrimitiveInterfaceCertificates.primitiveSeedReadback_resource_certificate`](formalization/generated/public_theorem_signatures.md#primitive-immutable-seed-readback-certificate).
- **Fixed endpoint generator program agreement.** [`PureSFormal.Computation.DeterministicTapePureSComputability.FixedEndpoint.generatorProgram_correct`](formalization/generated/public_theorem_signatures.md#fixed-endpoint-generator-program-agreement).
- **Fixed endpoint generator is primitive recursive.** [`PureSFormal.Computation.DeterministicTapePureSComputability.FixedEndpoint.generatorCode_primitiveRecursive`](formalization/generated/public_theorem_signatures.md#fixed-endpoint-generator-is-primitive-recursive).
- **Fixed endpoint generator is computable.** [`PureSFormal.Computation.DeterministicTapePureSComputability.FixedEndpoint.generatorCode_computable`](formalization/generated/public_theorem_signatures.md#fixed-endpoint-generator-is-computable).
- **Tape encoder primitive-recursive transfer.** [`PureSFormal.Computation.DeterministicTapePureSComputability.FixedEndpoint.encodeTermCode_primitiveRecursive_of_encodeBitsCode`](formalization/generated/public_theorem_signatures.md#tape-encoder-primitive-recursive-transfer).
- **Tape encoder computability transfer.** [`PureSFormal.Computation.DeterministicTapePureSComputability.FixedEndpoint.encodeTermCode_computable_of_encodeBitsCode_primitiveRecursive`](formalization/generated/public_theorem_signatures.md#tape-encoder-computability-transfer).

### B.5 Theorem 4 {#formal-theorem-4}

- **Generic confluence obstruction to exclusive branching.** [`PureSFormal.Research.ProtectedTrieConfluenceObstruction.no_exclusive_fork_decoder_of_confluent`](formalization/generated/public_theorem_signatures.md#generic-confluence-obstruction-to-exclusive-branching).
- **Generic confluence obstruction to distinct terminal forks.** [`PureSFormal.Research.ProtectedTrieConfluenceObstruction.no_distinct_terminal_fork_decoder_of_confluent`](formalization/generated/public_theorem_signatures.md#generic-confluence-obstruction-to-distinct-terminal-forks).

### B.6 Theorem 5 {#formal-theorem-5}

- **Persistent certificate enumeration.** [`PureSFormal.Research.ProtectedTrieCertificateEnumeration.protectedPersistentCertificateEnumeration`](formalization/generated/public_theorem_signatures.md#persistent-certificate-enumeration).
- **Exact terminal-observation bridge.** [`PureSFormal.StrongMultiwayUniversality.strongTerminalObservationIff`](formalization/generated/public_theorem_signatures.md#exact-terminal-observation-bridge).
- **Whole-observer resource certificate.** [`PureSFormal.Research.ProtectedTrieWholeObserverExactCost.wholeObserver_resource_certificate`](formalization/generated/public_theorem_signatures.md#whole-observer-resource-certificate).

### B.7 Supporting declarations {#appendix-b-7}

- **Finite bottom-up tree automaton representation.** [`PureSFormal.Research.FiniteTreeAutomatonPowerset.Deterministic`](formalization/generated/public_theorem_signatures.md#finite-bottom-up-tree-automaton-representation).
- **Finite marker observer all-input contract.** [`PureSFormal.Research.RootResetFiniteMarkerObserver.observer_all_input`](formalization/generated/public_theorem_signatures.md#finite-marker-observer-all-input-contract).
- **Finite marker observer makes no contraction.** [`PureSFormal.Research.RootResetFiniteMarkerObserver.observer_readOnly`](formalization/generated/public_theorem_signatures.md#finite-marker-observer-makes-no-contraction).
- **Native S contraction determines its occurrence.** [`PureSFormal.Research.PureSContractionOccurrenceInjectivity.contractAt_address_injective`](formalization/generated/public_theorem_signatures.md#native-s-contraction-determines-its-occurrence).
- **Physical fresh-field contraction iff eventual CTS emptiness.** [`PureSFormal.PureS.PhysicalMarkerExclusion.physicalEvent_iff_eventuallyEmpty`](formalization/generated/public_theorem_signatures.md#physical-fresh-field-contraction-iff-eventual-cts-emptiness).
- **Root marker observation iff eventual CTS emptiness.** [`PureSFormal.Research.RootResetMarkerObserverSoundness.root_observed_iff_eventuallyEmpty`](formalization/generated/public_theorem_signatures.md#root-marker-observation-iff-eventual-cts-emptiness).
- **Tree automaton agrees with the finite observer on every tree.** [`PureSFormal.Research.RootResetMarkerTreeAutomaton.automaton_accepts_eq_observer`](formalization/generated/public_theorem_signatures.md#tree-automaton-agrees-with-the-finite-observer-on-every-tree).
- **Fixed regular halting automaton.** [`PureSFormal.Research.RootResetRegularMarkerLanguage.automaton`](formalization/generated/public_theorem_signatures.md#fixed-regular-halting-automaton).
- **Fixed regular language detects source halting.** [`PureSFormal.Research.RootResetRegularMarkerLanguage.fixed_language_halting_iff`](formalization/generated/public_theorem_signatures.md#fixed-regular-language-detects-source-halting).
- **Infinite-tape initial representation.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.initial_represents`](formalization/generated/public_theorem_signatures.md#infinite-tape-initial-representation).
- **Infinite-tape forward step agreement.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.step_forward`](formalization/generated/public_theorem_signatures.md#infinite-tape-forward-step-agreement).
- **Infinite-tape backward step agreement.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.step_backward`](formalization/generated/public_theorem_signatures.md#infinite-tape-backward-step-agreement).
- **Infinite-tape undefined-step agreement.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.undefined_step_iff`](formalization/generated/public_theorem_signatures.md#infinite-tape-undefined-step-agreement).
- **Infinite-tape forward exact-fuel run.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.run_forward`](formalization/generated/public_theorem_signatures.md#infinite-tape-forward-exact-fuel-run).
- **Infinite-tape backward exact-fuel run.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.run_backward`](formalization/generated/public_theorem_signatures.md#infinite-tape-backward-exact-fuel-run).
- **Infinite-tape halting agreement.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.halts_iff`](formalization/generated/public_theorem_signatures.md#infinite-tape-halting-agreement).
- **Infinite-tape returned scanned-bit agreement.** [`PureSFormal.Computation.DeterministicTapeInfiniteTape.returns_iff`](formalization/generated/public_theorem_signatures.md#infinite-tape-returned-scanned-bit-agreement).
- **Challenge persistent-cursor contract.** [`PureSFormal.Challenge.ChallengePathUniversality`](formalization/generated/public_theorem_signatures.md#challenge-persistent-cursor-contract).
- **Challenge persistent-cursor universality theorem.** [`PureSFormal.Challenge.sCombinatorIsChallengePathUniversal`](formalization/generated/public_theorem_signatures.md#challenge-persistent-cursor-universality-theorem).
- **Challenge contract: fixed period.** [`PureSFormal.Challenge.ChallengePathUniversality.programPeriod`](formalization/generated/public_theorem_signatures.md#challenge-contract-fixed-period).
- **Challenge contract: duplicate-free controller cover.** [`PureSFormal.Challenge.ChallengePathUniversality.controllerStatesNoDuplicates`](formalization/generated/public_theorem_signatures.md#challenge-contract-duplicate-free-controller-cover).
- **Challenge contract: exhaustive controller cover.** [`PureSFormal.Challenge.ChallengePathUniversality.controllerCoverComplete`](formalization/generated/public_theorem_signatures.md#challenge-contract-exhaustive-controller-cover).
- **Challenge contract: textbook controller agreement.** [`PureSFormal.Challenge.ChallengePathUniversality.controllerTextbookAgreement`](formalization/generated/public_theorem_signatures.md#challenge-contract-textbook-controller-agreement).
- **Challenge contract: fixed endpoint counts.** [`PureSFormal.Challenge.ChallengePathUniversality.fixedEndpointCounts`](formalization/generated/public_theorem_signatures.md#challenge-contract-fixed-endpoint-counts).
- **Challenge contract: realization of every input.** [`PureSFormal.Challenge.ChallengePathUniversality.realizesEveryInput`](formalization/generated/public_theorem_signatures.md#challenge-contract-realization-of-every-input).
- **Challenge contract: every sampled edge is pure S.** [`PureSFormal.Challenge.ChallengePathUniversality.everySampledEdgeIsS`](formalization/generated/public_theorem_signatures.md#challenge-contract-every-sampled-edge-is-pure-s).
- **Challenge contract: strictly increasing checkpoints.** [`PureSFormal.Challenge.ChallengePathUniversality.checkpointsStrictlyIncrease`](formalization/generated/public_theorem_signatures.md#challenge-contract-strictly-increasing-checkpoints).
- **Challenge contract: exact checkpoint decoding.** [`PureSFormal.Challenge.ChallengePathUniversality.exactCTSDecoding`](formalization/generated/public_theorem_signatures.md#challenge-contract-exact-checkpoint-decoding).
- **Challenge contract: fixed tape-halting endpoint.** [`PureSFormal.Challenge.ChallengePathUniversality.fixedTapeHaltingEndpoint`](formalization/generated/public_theorem_signatures.md#challenge-contract-fixed-tape-halting-endpoint).
- **Challenge contract: fixed tape-encoder shape.** [`PureSFormal.Challenge.ChallengePathUniversality.fixedTapeEncoderShape`](formalization/generated/public_theorem_signatures.md#challenge-contract-fixed-tape-encoder-shape).
- **Challenge contract: linear cyclic-tag encoder size.** [`PureSFormal.Challenge.ChallengePathUniversality.ctsEncoderSize`](formalization/generated/public_theorem_signatures.md#challenge-contract-linear-cyclic-tag-encoder-size).
- **Challenge contract: computable cyclic-tag encoder.** [`PureSFormal.Challenge.ChallengePathUniversality.ctsEncoderComputable`](formalization/generated/public_theorem_signatures.md#challenge-contract-computable-cyclic-tag-encoder).
- **Challenge contract: bounded next contraction.** [`PureSFormal.Challenge.ChallengePathUniversality.nextContractionBound`](formalization/generated/public_theorem_signatures.md#challenge-contract-bounded-next-contraction).
- **Challenge contract: total selector decision.** [`PureSFormal.Challenge.ChallengePathUniversality.selectorTotalDecision`](formalization/generated/public_theorem_signatures.md#challenge-contract-total-selector-decision).
- **Challenge contract: bounded decoder.** [`PureSFormal.Challenge.ChallengePathUniversality.decoderBound`](formalization/generated/public_theorem_signatures.md#challenge-contract-bounded-decoder).
- **Challenge contract: bounded detector.** [`PureSFormal.Challenge.ChallengePathUniversality.detectorBound`](formalization/generated/public_theorem_signatures.md#challenge-contract-bounded-detector).
- **Challenge contract: periodicity without S.** [`PureSFormal.Challenge.ChallengePathUniversality.withoutSIsEventuallyPeriodic`](formalization/generated/public_theorem_signatures.md#challenge-contract-periodicity-without-s).
- **Contextual pure-S contraction.** [`PureSFormal.PureS.Step`](formalization/generated/public_theorem_signatures.md#contextual-pure-s-contraction).
- **Textbook pure-S rule agreement.** [`PureSFormal.PureS.step_iff_textbookStep`](formalization/generated/public_theorem_signatures.md#textbook-pure-s-rule-agreement).
- **Exact concrete scheduler checkpoints.** [`PureSFormal.PureS.SchedulerRecurrence.exactCheckpoint`](formalization/generated/public_theorem_signatures.md#exact-concrete-scheduler-checkpoints).
- **Fixed cyclic-tag program period.** [`PureSFormal.Cook.rogozhinCookProgram_period`](formalization/generated/public_theorem_signatures.md#fixed-cyclic-tag-program-period).
- **Cyclic-tag phase arithmetic.** [`PureSFormal.CTS.iteratePhase_val`](formalization/generated/public_theorem_signatures.md#cyclic-tag-phase-arithmetic).
- **Deterministic tape to primitive three-counter halting.** [`PureSFormal.Computation.DeterministicTapeThreeCounterCompiler.Execution.halts_iff_threeCounterHalts`](formalization/generated/public_theorem_signatures.md#deterministic-tape-to-primitive-three-counter-halting).
- **Primitive three-counter to restricted tag halting.** [`PureSFormal.Computation.ThreeCounterTag.Numeric.universalHalts_iff_compileJob`](formalization/generated/public_theorem_signatures.md#primitive-three-counter-to-restricted-tag-halting).
- **Restricted tag to Rogozhin-machine halting.** [`PureSFormal.Computation.RogozhinT2Simulation.eventuallyHalts_compile_iff`](formalization/generated/public_theorem_signatures.md#restricted-tag-to-rogozhin-machine-halting).
- **Rogozhin machine to fixed cyclic-tag emptiness.** [`PureSFormal.Cook.PassClassification.canonical_eventuallyHalts_iff_exists_cts_empty`](formalization/generated/public_theorem_signatures.md#rogozhin-machine-to-fixed-cyclic-tag-emptiness).
- **Fixed cyclic-tag emptiness to marked-checkpoint observation.** [`PureSFormal.PureS.TermEvent.eventuallyObservesMarkedCheckpointRaw_iff_eventuallyEmpty`](formalization/generated/public_theorem_signatures.md#fixed-cyclic-tag-emptiness-to-marked-checkpoint-observation).
- **Deterministic tape to compiled three-counter job.** [`PureSFormal.Computation.DeterministicTapeCook.halts_iff_threeCounterJob`](formalization/generated/public_theorem_signatures.md#deterministic-tape-to-compiled-three-counter-job).
- **Deterministic tape to compiled restricted tag job.** [`PureSFormal.Computation.DeterministicTapeCook.halts_iff_t2Job`](formalization/generated/public_theorem_signatures.md#deterministic-tape-to-compiled-restricted-tag-job).
- **Literal decoder factorization.** [`PureSFormal.PureS.PublicDecoder.decode_eq_decodeLiteral`](formalization/generated/public_theorem_signatures.md#literal-decoder-factorization).
- **Marked-checkpoint detector characterization.** [`PureSFormal.PureS.TermEvent.emitsMarkH?_eq_true_iff`](formalization/generated/public_theorem_signatures.md#marked-checkpoint-detector-characterization).
- **Selected-path global raw-microtick bound.** [`PureSFormal.WeakPathUniversality.finiteCTS_sampleTick_le_explicit`](formalization/generated/public_theorem_signatures.md#selected-path-global-raw-microtick-bound).
- **Quadratic-log source-horizon transfer.** [`PureSFormal.WeakPathPolynomialCost.checkpointTime_le_of_horizon_le_quadratic_log`](formalization/generated/public_theorem_signatures.md#quadratic-log-source-horizon-transfer).
- **Mutation-free cursor periodicity.** [`PureSFormal.PureS.FiniteController.eventually_periodic_of_mutation_free`](formalization/generated/public_theorem_signatures.md#mutation-free-cursor-periodicity).
- **Bounded next contraction on every realized sample.** [`PureSFormal.EvaluatorBounds.selectNext_on_run_halts_within`](formalization/generated/public_theorem_signatures.md#bounded-next-contraction-on-every-realized-sample).
- **All-configuration bounded selector decision.** [`PureSFormal.EvaluatorBounds.selectNext_total_decision`](formalization/generated/public_theorem_signatures.md#all-configuration-bounded-selector-decision).
- **Contraction-disabled controller periodicity.** [`PureSFormal.EvaluatorBounds.withoutContraction_run_eventuallyPeriodic`](formalization/generated/public_theorem_signatures.md#contraction-disabled-controller-periodicity).
- **Counted decoder value agreement.** [`PureSFormal.PureS.CountedCheckpointDecoder.countedDecode_value`](formalization/generated/public_theorem_signatures.md#counted-decoder-value-agreement).
- **Counted decoder all-input bound.** [`PureSFormal.PureS.CountedCheckpointDecoder.countedDecode_ticks_le`](formalization/generated/public_theorem_signatures.md#counted-decoder-all-input-bound).
- **Counted detector value agreement.** [`PureSFormal.PureS.CountedTermEvent.countedObservesMarkedCheckpoint?_value`](formalization/generated/public_theorem_signatures.md#counted-detector-value-agreement).
- **Counted detector all-input bound.** [`PureSFormal.PureS.CountedTermEvent.countedObservesMarkedCheckpoint?_ticks_le`](formalization/generated/public_theorem_signatures.md#counted-detector-all-input-bound).
- **Fixed endpoint raw cover counts.** [`PureSFormal.Computation.FixedEndpointUniformity.fixedEndpoint_rawCoverCounts`](formalization/generated/public_theorem_signatures.md#fixed-endpoint-raw-cover-counts).
- **Fixed controller has a duplicate-free cover.** [`PureSFormal.Computation.FixedEndpointUniformity.fixedController_states_nodup`](formalization/generated/public_theorem_signatures.md#fixed-controller-has-a-duplicate-free-cover).
- **Fixed controller cover is exhaustive.** [`PureSFormal.Computation.FixedEndpointUniformity.fixedController_covers`](formalization/generated/public_theorem_signatures.md#fixed-controller-cover-is-exhaustive).
- **Fixed controller agrees with the textbook tree-walker.** [`PureSFormal.Computation.FixedEndpointUniformity.fixedController_textbookAgreement`](formalization/generated/public_theorem_signatures.md#fixed-controller-agrees-with-the-textbook-tree-walker).
- **Fixed endpoint input dependence.** [`PureSFormal.Computation.FixedEndpointUniformity.fixedEncoder_eq_generator`](formalization/generated/public_theorem_signatures.md#fixed-endpoint-input-dependence).
- **Initial bare-root classifier agreement.** [`PureSFormal.Research.RootResetPersistentInitialBridge.firstPersistentEdge_rootResetAgreement`](formalization/generated/public_theorem_signatures.md#initial-bare-root-classifier-agreement).
- **Every defined tape run has a decoded primitive boundary.** [`PureSFormal.Computation.DeterministicTapeTrajectoryDecoder.exists_decodedPrimitiveBoundary_of_runFor?_eq_some`](formalization/generated/public_theorem_signatures.md#every-defined-tape-run-has-a-decoded-primitive-boundary).
- **Retained tape-compiler exponential primitive lower bound.** [`PureSFormal.Computation.LegacyCompilerGrowth.PrimitiveClock.two_pow_le_rightPopClock_replicate_false`](formalization/generated/public_theorem_signatures.md#retained-tape-compiler-exponential-primitive-lower-bound).
- **Retained compiled-boundary double-exponential lower bound.** [`PureSFormal.Computation.LegacyCompilerGrowth.TagMaterialization.two_pow_two_pow_le_compiled_boundary_encodeState`](formalization/generated/public_theorem_signatures.md#retained-compiled-boundary-double-exponential-lower-bound).
- **Shared-store kernel certificate.** [`PureSFormal.CostModel.sharedCostKernelCertificate`](formalization/generated/public_theorem_signatures.md#shared-store-kernel-certificate).
- **Concrete finite-arena interpreter certificate.** [`PureSFormal.CostModel.FiniteArena.interpreterCertificate`](formalization/generated/public_theorem_signatures.md#concrete-finite-arena-interpreter-certificate).
- **Unified selected-path resource ledger.** [`PureSFormal.CostModel.UnifiedResourceLedger.finiteCTS_endToEndLedger`](formalization/generated/public_theorem_signatures.md#unified-selected-path-resource-ledger).
- **Logarithmic pointer-word representation.** [`PureSFormal.CostModel.FiniteArena.pointerWordCertificate`](formalization/generated/public_theorem_signatures.md#logarithmic-pointer-word-representation).

### B.8 Appendix F declarations {#formal-appendix-f}

- **F.1: Autonomous-component obstruction.** [`PureSFormal.AppendixF.AutonomousObstruction.autonomous_obstruction`](formalization/generated/public_theorem_signatures.md#f1-autonomous-component-obstruction).
- **F.1: Main outer-clock corollary.** [`PureSFormal.AppendixF.AutonomousObstruction.clock_encoder_obstruction`](formalization/generated/public_theorem_signatures.md#f1-main-outer-clock-corollary).
- **F.2.1: Decidable-path obstruction.** [`PureSFormal.AppendixF.DecidablePathObstruction.decidable_path_obstruction`](formalization/generated/public_theorem_signatures.md#f21-decidable-path-obstruction).
- **F.2.1: Finite unions and normal forms.** [`PureSFormal.AppendixF.PathFamilies.finite_union_obstruction`](formalization/generated/public_theorem_signatures.md#f21-finite-unions-and-normal-forms).
- **F.3.1: Native carrier blocks.** [`PureSFormal.AppendixF.Carrier.Configuration.native`](formalization/generated/public_theorem_signatures.md#f31-native-carrier-blocks).
- **F.3.1: Infinite native carrier path.** [`PureSFormal.AppendixF.Carrier.Configuration.run_step`](formalization/generated/public_theorem_signatures.md#f31-infinite-native-carrier-path).
- **F.3.1: Exact carrier observation decision.** [`PureSFormal.AppendixF.Carrier.eventuallyAccepts_path_iff`](formalization/generated/public_theorem_signatures.md#f31-exact-carrier-observation-decision).
- **F.3.1: Finite-summary bound.** [`PureSFormal.AppendixF.Carrier.Summary.enumeration_bound`](formalization/generated/public_theorem_signatures.md#f31-finite-summary-bound).
- **F.3: Unique carrier recognition.** [`PureSFormal.AppendixF.Carrier.parse_iff`](formalization/generated/public_theorem_signatures.md#f3-unique-carrier-recognition).
- **F.3.2: Regular avoidance.** [`PureSFormal.AppendixF.Carrier.regular_avoidance`](formalization/generated/public_theorem_signatures.md#f32-regular-avoidance).
- **F.3.3: AAA boundary.** [`PureSFormal.AppendixF.AAABoundary.aaa_boundary`](formalization/generated/public_theorem_signatures.md#f33-aaa-boundary).
- **F.4.1: Native recursive feedback.** [`PureSFormal.AppendixF.RecursiveCall.Configuration.native`](formalization/generated/public_theorem_signatures.md#f41-native-recursive-feedback).
- **F.4.1: Unique-redex hold-only path.** [`PureSFormal.AppendixF.RecursiveCall.hold_all_data`](formalization/generated/public_theorem_signatures.md#f41-unique-redex-hold-only-path).
- **F.4.2: Exact context and data update.** [`PureSFormal.AppendixF.RecursiveCall.observation_next`](formalization/generated/public_theorem_signatures.md#f42-exact-context-and-data-update).
- **F.4.2: Exact intermediate observations.** [`PureSFormal.AppendixF.RecursiveCall.observation_accepts`](formalization/generated/public_theorem_signatures.md#f42-exact-intermediate-observations).
- **F.4.3: Recursive-call observation decision.** [`PureSFormal.AppendixF.RecursiveCall.eventuallyAccepts_path_iff`](formalization/generated/public_theorem_signatures.md#f43-recursive-call-observation-decision).
- **F.4.4: Incomparable basins.** [`PureSFormal.AppendixF.IncomparableBasins.incomparable`](formalization/generated/public_theorem_signatures.md#f44-incomparable-basins).
- **F.4: Classical seven-contraction example.** [`PureSFormal.AppendixF.ClassicalSeeds.pp_seven_steps`](formalization/generated/public_theorem_signatures.md#f4-classical-seven-contraction-example).
- **F.1: Unique-redex clock.** [`PureSFormal.AppendixF.ClockBoundary.clock_one_redex_all`](formalization/generated/public_theorem_signatures.md#f1-unique-redex-clock).
- **F.4: Recognition of arbitrary supplied tables.** [`PureSFormal.AppendixF.RecursiveCall.parseCall_covers_table`](formalization/generated/public_theorem_signatures.md#f4-recognition-of-arbitrary-supplied-tables).
- **F.4: Finite-table extraction.** [`PureSFormal.AppendixF.RecursiveCall.recognized_call_has_finite_table`](formalization/generated/public_theorem_signatures.md#f4-finite-table-extraction).
- **F.2: Standard computability of certificate search.** [`PureSFormal.AppendixF.ComputabilityCertificates.computable_iff_certificate`](formalization/generated/public_theorem_signatures.md#f2-standard-computability-of-certificate-search).

### B.9 Scheduler grammars and exact sampled execution {#appendix-b-9}
...
### B.10 Worked-example cost declarations {#appendix-b-10}

The 22-contraction decomposition in [Appendix E.3](#appendix-e-3) is the specialization of
...
In [Theorem 3](#theorem-3), $\Lambda_P$ is `allActionLabels P`, $r_P$ is the route field of
...
### B.11 Primitive resource derivation {#primitive-resource-derivation}

All costs below count the primitive operations of [Theorem 3](#theorem-3). Returning an
...
and six operations. These additions give the table and [Equations (27)](#equation-7)--[(28)](#equation-8).
...
#### B.11.1 Counted clauses behind the 6144 and 5124 bounds {#appendix-b-11-1}
...
#### B.11.2 Small recursive machines {#appendix-b-11-2}
...
#### B.11.3 Parsing the fixed 114-bit Cook alphabet {#appendix-b-11-3}
...
#### B.11.4 Reading unary run counts and radix-eight stacks {#appendix-b-11-4}
...
#### B.11.5 Canonical boundaries and positive-horizon arrivals {#appendix-b-11-5}
...
#### B.11.6 Program-frame decoding and the full 5124 seed adapter {#appendix-b-11-6}
...
where $v$ is the source input and $c$ is its stack encoding from [Section 6](#section-6).
...
#### B.11.7 Literal 127-phase source table {#appendix-b-11-7}
...
#### B.11.8 Typed productions: the coefficient 142 {#appendix-b-11-8}
...
#### B.11.9 Numeric symbols and the coefficients 7759 and 7999 {#appendix-b-11-9}
...
#### B.11.10 Literal register blocks in the emitted initial word {#appendix-b-11-10}
...
#### B.11.11 Source-code decoding and the coefficients 138, 246 and 102 {#appendix-b-11-11}
...
#### B.11.12 Padding and literal initial-stack construction {#appendix-b-11-12}
...
#### B.11.13 Deletion-two normalization {#appendix-b-11-13}
...
#### B.11.14 Rogozhin weights, data and program serialization {#appendix-b-11-14}
...
#### B.11.15 Cook serialization and the pure-S output {#appendix-b-11-15}
...
## Appendix C. Unrestricted reduction and certificate histories {#appendix-c}
...
table. This observer boundary is one of [Theorem 5](#theorem-5)'s eight clauses.

### C.1 Theorem 4. Confluence obstruction to exclusive functional decoding {#theorem-4}
...
E\leadsto T\leadsto U
  &\Longrightarrow d(T)\Rightarrow^*d(U),\tag{32}\\
i\Rightarrow^*x
  &\Longrightarrow
    \exists T\;\bigl(E\leadsto T\land d(T)=x\bigr).\tag{33}
\end{align}
...
decoded join a common source continuation, contradicting [Equation (30)](#equation-16).
Distinct reachable source sinks are the immediate special case. [Appendix B](#appendix-b)
...
**Kernel correspondence.** [Appendix B.5](#formal-theorem-4) lists the checked declarations. The public ledger gives their complete types and exact axiom sets.
...
### C.2 Theorem 5. Persistent exposure and enumeration of observer-verified histories {#theorem-5}
...
**Kernel correspondence.** [Appendix B.6](#formal-theorem-5) lists the checked declarations. The public ledger gives their complete types and exact axiom sets.
...
uses that header when checking adjacent rows. [Appendix C](#appendix-c) gives the protected
address algebra, directed subdivision, and adjacent-step enumeration proof.

### C.3 Source histories and persistent states {#appendix-c-3}
...
edges remain distinct ([`strongStateMergedConfigurationQuotient`](formalization/generated/public_theorem_signatures.md#state-merged-configuration-quotient)).

### C.4 Source-independent protected generation {#appendix-c-4}
...
### C.5 Prefix-free addresses and literal tableaux {#appendix-c-5}
...
### C.6 Projection and every-edge persistence {#appendix-c-6}

**From protected addresses to the eight clauses.** Fix the finite ordered-binary
source instance $X$ in [Theorem 5](#theorem-5). Retain the protected-node, opening, and
...
### C.7 Exact range and cofinal extension {#appendix-c-7}
...
cofinality \[[49](#reference-49)\], pp. 5, 27--28. It secures every copy created by duplication;
...
Such continuations exist: concatenate complete-development rounds \[[49](#reference-49)\],
...
The six- or seven-contraction traces of [Appendix C.4](#appendix-c-4) concatenate to an
...
### C.8 Adjacent-step path and history-tree subdivision {#appendix-c-8}
...
### C.9 Observer boundary and resources {#appendix-c-9}
...
([`labelledProjection_literal_final_on_cone`](formalization/generated/public_theorem_signatures.md#literal-final-row-provenance),
[`labelledProjection_slot_and_terminal_on_cone`](formalization/generated/public_theorem_signatures.md#ordered-enabled-slots-and-terminality)).
[`wholeObserver_resource_certificate`](formalization/generated/public_theorem_signatures.md#whole-observer-resource-certificate) proves, for unfolded term size $N$, at
most $512(N+1)^5$ structural ticks, $512(N+1)^2$ structural peak, and
$10(N+1)^2$ output cells.

#### C.9.1 Structural observer meter clauses {#appendix-c-9-1}
...
#### C.9.2 Source decoding: the coefficient 50 {#appendix-c-9-2}
...
#### C.9.3 Tableau parser and literal transition validation {#appendix-c-9-3}
...
#### C.9.4 Protected paths, candidate parsing and collection {#appendix-c-9-4}
...
#### C.9.5 The public 512 and output 10 coefficients {#appendix-c-9-5}
...
#### C.9.6 The structural encoder's coefficients 32 and 3 {#appendix-c-9-6}
...
### C.10 Terminal observation {#appendix-c-10}
...
([`terminalCertificateEquivalence`](formalization/generated/public_theorem_signatures.md#terminal-certificate-equivalence), [`strongTerminalObservationIff`](formalization/generated/public_theorem_signatures.md#exact-terminal-observation-bridge)). Finite
redex enumeration and bounded reduction layers give the separate
semidecision theorem [`strongTerminalObservationSemidecidable`](formalization/generated/public_theorem_signatures.md#terminal-observation-is-semidecidable). These results
use the observer boundary stated in [Appendix C.9](#appendix-c-9); the protected body supplies the
permanent candidate space.

\Needspace{22\baselineskip}

## Appendix D. Fixed Rogozhin table and compiler conventions {#appendix-d}
...
$A,\ldots,D$. The table is Rogozhin's \[[25](#reference-25)\], section 8, p. 232, under the
...
conventions and the production formulas in [Appendix D.8](#app-d-8) specify the
fixed program. [Appendix D.9](#app-d-9) gives every one of its 912 appendants
...
fixed table in [Appendix D](#appendix-d)):
...
## Appendix E. Persistent-controller traces and finite-control counts {#appendix-e}

### E.1 The complete two-phase input {#appendix-e-1}
...
Expanding the abbreviations in [Equation (35)](#equation-w1) leaves only $\mathbf S$ and
application and gives exactly 171 unshared nodes. The executable constructors
compute these same subterms; [Appendix B](#appendix-b) maps them to the formal source.



### E.2 Persistent microticks before the first contraction {#appendix-e-2}
...
### E.3 The persistent 22-contraction response {#appendix-e-3}
...
The stage, clock, job, fuel, and base rows use the constructors of [Section 4.2](#section-4-2).
The deletion, response, dispatcher, and append rows use the local identities
and checkpoint argument of [Section 4.7](#section-4-7). [Section 5.2](#section-5-2) proves that fresh-root
...
two contractions instantiate [Equation (10)](#equation-12). Write $V$ for the active queue
...
### E.4 The persistent control cover {#appendix-e-4}
...
[Theorem 2](#theorem-2) fixes the 912-phase program, transition table, initial control,
...
[Appendix B](#appendix-b) identifies the fixedness and count certificates. The universal
dynamics consist of pure-$\mathbf S$ contractions, this fixed finite control,
and one persistent cursor in this baseline. [Theorem 1R](#theorem-1r) supplies the final
root-restarted controller with its own finite cover and coefficient.

## Appendix F. Comparison of computation models {#appendix-f}
...
axiom dependencies. [Appendix B](#appendix-b) includes the declaration map. The following
...
### F.1 Selected paths and unrestricted reduction {#appendix-f-1}
...
\[[1](#reference-1)\]. This paper proves the selected-evaluation-path formulation: a single
...
|------------------------|----------------------------------------------------------------------------|
| Evaluation order | One fixed finite controller starts at the root for each contraction; only the current bare term passes between invocations ([Theorem 1R](#theorem-1r)). |
...
| Universal source and output | [Theorem 2](#theorem-2) and [Corollary 2.1](#corollary-2-1) give one fixed 912-phase endpoint that preserves and reflects deterministic Boolean-tape halting, literal source rows and terminal scanned-bit output. [Theorem 1R](#theorem-1r) transfers all of them to the fresh-root controller. |
...
| Checkability | The universal construction and all-input correctness statements of Theorems 1--3 and 1R and [Corollary 2.1](#corollary-2-1) are kernel-checked. Bounded worked traces exercise the implementations. [Section 7.4](#section-7-4) states the scope of the runtime demonstrations. |
...
|-------:|-------------------------------------|-------------------------------------------------------|
| 1 | normal-form output | unavailable as a computable halting target because normalizability is decidable \[[42](#reference-42)\], theorem 8.6.1 |
| 2 | forward-sound, source-complete single-valued decoding across all reducts | impossible for a reachable nonjoinable source fork and a confluent target, by [Theorem 4](#theorem-4) |
| 3 | set-valued observation of unrestricted reduction | [Theorem 5](#theorem-5) gives observer-verified histories |
...
| **6** | **term-only root-restarting finite selector** | **proved by [Theorem 1R](#theorem-1r), including all-input linear termination** |
...
descendants having no common continuation, [Theorem 4](#theorem-4) excludes the decoder
...
The separate unrestricted-reduction result, [Theorem 5](#theorem-5), concerns
...
\[[10](#reference-10)\], pp. 12--13 of the author manuscript. The obstruction therefore
...
residual-fair scheduling, since the chosen path can be unfair. [Theorem 1R](#theorem-1r)
uses its specified selector and requires neither conclusion.

\Needspace{16\baselineskip}

### F.2 Finite observation and preprocessing {#appendix-f-2}
...
$\mathcal A$ or a normal form, then $E$ cannot satisfy [Appendix F.1](#appendix-f-1)'s exact
...
exact equivalence in [Appendix F.1](#appendix-f-1) identifies its acceptance with
...
states already provide nontermination proofs \[[6](#reference-6)\], Definition 1 and
Theorems 2 and 37. Liveness-to-termination transformations also treat
regular target languages \[[50](#reference-50)\]. The completeness directions in Theorems 2
...
produce infinitely many most-general patterns \[[45](#reference-45)\], Definition 5.6.8,
Algorithm 5.6.13 and theorem 5.6.17. The automata for generalized combinators in \[[22](#reference-22)\],
...
### F.3 Tagged carriers and regular observation {#appendix-f-3}

The classical carrier clock \[[5](#reference-5)\], pp. 2--3; \[[10](#reference-10)\], author manuscript,
...
with the input. We use the observer notation of [Appendix F.2](#appendix-f-2).

#### F.3.1 An effective observation algorithm {#appendix-f-3-1}
...
#### F.3.2 A regular language of avoiding occurrences {#appendix-f-3-2}
...
methods \[[6](#reference-6)\], pp. 162--163 and 172--173. The explicit summaries above
...
#### F.3.3 A boundary under every native contraction {#appendix-f-3-3}

AAA's divergence and regular-invariant proofs are classical \[[10](#reference-10)\], author
...
### F.4 Recursive I/G calls and regular observation {#appendix-f-4}

This section uses the observer notation and exact every-path target of
[Appendix F.2](#appendix-f-2). Its program table, data and surrounding context may all depend
...
$P=I^2(S)=S(SSS)S$, Waldmann \[[5](#reference-5)\], Example 66, printed p. 16, gives $PP$
...
basin of [Appendix F.3](#appendix-f-3) and the recursive-call-or-normal-form basin above
are incomparable.
...
[Appendix F.3](#appendix-f-3) carrier has two head arguments and is either
...
Here $C_0=R_{S,S}$ and $C_{n+1}=N_S(C_n)$ are [Appendix F.3](#appendix-f-3) carriers.
...
### F.5 Exact scope of the claims {#appendix-f-5}
...
| Does the selector restart at the root and depend only on the current bare term? | Yes. [Theorem 1R](#theorem-1r) proves a fixed finite machine with every-input termination and exact every-contraction path identity. |
| Is every deterministic tape row decoded along the pure-$\mathbf S$ path? | Yes, under the effective state-padding encoder of [Corollary 2.1](#corollary-2-1). Fixed bare-term observers accept exactly reachable source rows and terminal outputs. |
| Is tape halting reduced to existence of a normal form? | No. The fixed detector recognizes a marked event on a continuing path; pure-$\mathbf S$ normalizability is decidable \[[42](#reference-42)\], theorem 8.6.1. |
...
| Is a premise-free polynomial source-time bound proved? | No. The polynomial composition in [Section 7.3](#section-7-3) assumes an explicit source-horizon inequality. |
| Does the unrestricted graph simulate source branching locally? | No. [Theorem 5](#theorem-5) proves persistent observer-verified history enumeration; the observer checks the source transition relation. |

[Corollary 2.1](#corollary-2-1) recovers static decoding information from the immutable seed
...
included in the complete quartic source-output bound stated in [Theorem 3](#theorem-3).
...
above and in [Appendix B](#appendix-b) identify their exact formal statements.
...
This appendix derives the constants in [Section 5.2.4](#section-5-2-4) from the finite
pattern tables and worker compositions of [Section 5.1](#section-5-1). All bounds
...
For pending admission, use the finite child rows of [Section 5.2.1](#active-copy-exclusion) and
...
in [Section 5.2.1](#active-copy-exclusion) select $L$ and $LL$. The pattern recurrence above gives
...
[Equation (13)](#equation-rootpotential). The entry boundary holds at the initial root
...
boundary proved in [Section 5.2.2](#response-decision); the scoped workers establish it by
...
table in [Section 5.2.2](#response-decision) gives the following exact declared composition
coefficients.
...
1. []{#reference-1}Stephen Wolfram, “1920, 2020 and a $20,000 Prize: Announcing the S
   Combinator Challenge,” *Stephen Wolfram Writings*, 8 June 2021.
   [doi:10.31855/7709b85c-8e6](https://doi.org/10.31855/7709b85c-8e6).

2. []{#reference-2}B. K. Rosen, “Tree-Manipulating Systems and Church-Rosser Theorems,”
   *Journal of the ACM* **20**(1) (1973), 160--187.
   [doi:10.1145/321738.321750](https://doi.org/10.1145/321738.321750).

3. []{#reference-3}Stephen Wolfram, “Combinators: A Centennial View,” 6 December 2020;
   arXiv:2103.12811, 2021.
   [doi:10.48550/arXiv.2103.12811](https://doi.org/10.48550/arXiv.2103.12811).

4. []{#reference-4}Farrokh Vatan, “On Universality of the S Combinator,” arXiv:2210.12893,
   2022. [doi:10.48550/arXiv.2210.12893](https://doi.org/10.48550/arXiv.2210.12893).

5. []{#reference-5}Johannes Waldmann, “The Combinator S,” *Information and Computation*
   **159**(1--2) (2000), 2--21.
   [doi:10.1006/inco.2000.2874](https://doi.org/10.1006/inco.2000.2874).

6. []{#reference-6}Jörg Endrullis and Hans Zantema, “Proving Non-Termination by Finite
   Automata,” in *Rewriting Techniques and Applications (RTA 2015)*,
   LIPIcs **36**, 160--176.
   [doi:10.4230/LIPIcs.RTA.2015.160](https://doi.org/10.4230/LIPIcs.RTA.2015.160).

7. []{#reference-7}Max Dauchet, “Simulation of Turing Machines by a Regular Rewrite
   Rule,” *Theoretical Computer Science* **103**(2) (1992), 409--420.
   [doi:10.1016/0304-3975(92)90022-8](<https://doi.org/10.1016/0304-3975(92)90022-8>).

8. []{#reference-8}Martin Kutrib and Uwe Meyer, “Deterministic Tree-Walking-Storage
   Automata,” *Acta Informatica* **63** (2026), Article 9.
   [doi:10.1007/s00236-026-00522-5](https://doi.org/10.1007/s00236-026-00522-5).

9. []{#reference-9}Gérard Huet, “The Zipper,” *Journal of Functional Programming* **7**(5)
   (1997), 549--554.
   [doi:10.1017/S0956796897002864](https://doi.org/10.1017/S0956796897002864).

10. []{#reference-10}Henk Barendregt, Jörg Endrullis, Jan Willem Klop, and Johannes
    Waldmann, “Dance of the Starlings,” in *Raymond Smullyan on Self
    Reference*, Outstanding Contributions to Logic **14**, Springer, 2018,
    67--111.
    [doi:10.1007/978-3-319-68732-2_5](https://doi.org/10.1007/978-3-319-68732-2_5).
    [Author manuscript](https://www.cs.ru.nl/~henk/BEKW.pdf).

11. []{#reference-11}Stephen Wolfram, “Multiway Turing Machines,” arXiv:2103.04961, 2021.
    [doi:10.48550/arXiv.2103.04961](https://doi.org/10.48550/arXiv.2103.04961).

12. []{#reference-12}David A. Turner, “A New Implementation Technique for Applicative
    Languages,” *Software: Practice and Experience* **9**(1) (1979), 31--49.
    [doi:10.1002/spe.4380090105](https://doi.org/10.1002/spe.4380090105).

13. []{#reference-13}Richard Kennaway, Jan Willem Klop, M. Ronan Sleep, and Fer-Jan de Vries,
    ...
    *ACM Transactions on Programming Languages and Systems* **16**(3) (1994),
    493--523. [doi:10.1145/177492.177577](https://doi.org/10.1145/177492.177577).

14. []{#reference-14}Martin Avanzini and Georg Moser, “Technical Report: Complexity Analysis by
    Graph Rewriting Revisited,” arXiv:1001.5404v3, submitted 8 June 2011.
    The arXiv record uses the title “Efficient Implementation of Rewriting
    Revisited Technical Report”; the v3 PDF title page is dated 21 June 2018.
    [doi:10.48550/arXiv.1001.5404](https://doi.org/10.48550/arXiv.1001.5404).
    [Version 3](https://arxiv.org/abs/1001.5404v3).

15. []{#reference-15}Andrea Corradini and Frank Drewes, “Term Graph Rewriting and Parallel
    Term Rewriting,” *Electronic Proceedings in Theoretical Computer Science*
    **48** (2011), 3--18.
    [doi:10.4204/EPTCS.48.3](https://doi.org/10.4204/EPTCS.48.3).

16. []{#reference-16}Ugo Dal Lago and Simone Martini, “Derivational Complexity Is an Invariant
    Cost Model,” in *Foundational and Practical Aspects of Resource Analysis*,
    LNCS **6324** (2010), 100--113.
    [doi:10.1007/978-3-642-15331-0_7](https://doi.org/10.1007/978-3-642-15331-0_7).

17. []{#reference-17}Olivier Danvy and Ian Zerny, “Three Syntactic Theories for Combinatory
    Graph Reduction,” *ACM Transactions on Computational Logic* **14**(4)
    (2013), Article 29.
    [doi:10.1145/2528932](https://doi.org/10.1145/2528932).

18. []{#reference-18}Beniamino Accattoli and Ugo Dal Lago, “(Leftmost-Outermost) Beta Reduction
    Is Invariant, Indeed,” *Logical Methods in Computer Science* **12**(1:4)
    (2016). [doi:10.2168/LMCS-12(1:4)2016](<https://doi.org/10.2168/LMCS-12(1:4)2016>).

19. []{#reference-19}Andrea Asperti and Harry G. Mairson, “Parallel Beta Reduction Is Not
    Elementary Recursive,” *Information and Computation* **170**(1) (2001),
    49--80. [doi:10.1006/inco.2001.2869](https://doi.org/10.1006/inco.2001.2869).

20. []{#reference-20}Martin Avanzini and Georg Moser, “Complexity of Acyclic Term Graph
    Rewriting,” in *1st International Conference on Formal Structures for
    Computation and Deduction (FSCD 2016)*, LIPIcs **52**, Article 10.
    [doi:10.4230/LIPIcs.FSCD.2016.10](https://doi.org/10.4230/LIPIcs.FSCD.2016.10).

21. []{#reference-21}Keisuke Nakano and Munehiro Iwami, “Disproving Termination of Non-erasing
    Sole Combinatory Calculus with Tree Automata,” in *Implementation and
    Application of Automata (CIAA 2024)*, LNCS **15015**, 261--275.
    [doi:10.1007/978-3-031-71112-1_19](https://doi.org/10.1007/978-3-031-71112-1_19).

22. []{#reference-22}Munehiro Iwami and Keisuke Nakano, “Disproving Termination of O and
    S-like Combinators by Tree Automata,” *Journal of Information Processing*
    **34** (2026), 39--44.
    [doi:10.2197/ipsjjip.34.39](https://doi.org/10.2197/ipsjjip.34.39).

23. []{#reference-23}Turlough Neary and Damien Woods, “P-Completeness of Cellular Automaton
    Rule 110,” in *Automata, Languages and Programming (ICALP 2006), Part I*,
    LNCS **4051**, Springer, 2006, 132--143.
    [doi:10.1007/11786986_13](https://doi.org/10.1007/11786986_13).

24. []{#reference-24}Turlough Neary, *Small Universal Turing Machines*, PhD thesis, National
    University of Ireland Maynooth, 2008,
    [Section 4.3, theorem 4.3.2](https://gwern.net/doc/cs/computable/2008-neary.pdf).

25. []{#reference-25}Yurii Rogozhin, “Small Universal Turing Machines,” *Theoretical Computer
    Science* **168**(2) (1996), 215--240.
    [doi:10.1016/S0304-3975(96)00077-1](<https://doi.org/10.1016/S0304-3975(96)00077-1>).

26. []{#reference-26}Matthew Cook, “Universality in Elementary Cellular Automata,” *Complex
    Systems* **15**(1) (2004), 1--40.
    [doi:10.25088/ComplexSystems.15.1.1](https://doi.org/10.25088/ComplexSystems.15.1.1).

27. []{#reference-27}John Cocke and Marvin Minsky, “Universality of Tag Systems with
    $P=2$,” *Journal of the ACM* **11**(1) (1964), 15--20.
    [doi:10.1145/321203.321206](https://doi.org/10.1145/321203.321206).

28. []{#reference-28}Marvin Minsky, *Computation: Finite and Infinite Machines*,
    Prentice-Hall, 1967, Chapter 14.

29. []{#reference-29}Oscar H. Ibarra and Nicholas Q. Trân, “A Note on Simple Programs with
    Two Variables,” *Theoretical Computer Science* **112**(2) (1993),
    391--397.
    [doi:10.1016/0304-3975(93)90028-R](<https://doi.org/10.1016/0304-3975(93)90028-R>).

30. []{#reference-30}Matthew Cook, “A Concrete View of Rule 110 Computation,” *Electronic
    Proceedings in Theoretical Computer Science* **1** (2009), 31--55.
    [doi:10.4204/EPTCS.1.4](https://doi.org/10.4204/EPTCS.1.4).

31. []{#reference-31}Glynn Winskel, “Event Structures,” in *Petri Nets: Applications and
    Relationships to Other Models of Concurrency*, LNCS **255**, Springer,
    1987, 325--392.
    [doi:10.1007/3-540-17906-2_31](https://doi.org/10.1007/3-540-17906-2_31).

32. []{#reference-32}Mikołaj Bojańczyk and Thomas Colcombet, “Tree-Walking Automata Do Not
    Recognize All Regular Languages,” *SIAM Journal on Computing* **38**(2)
    (2008), 658--701.
    [doi:10.1137/050645427](https://doi.org/10.1137/050645427).

33. []{#reference-33}Jeroen Fokker, “The Systematic Construction of a One-Combinator Basis
    for Lambda-Terms,” *Formal Aspects of Computing* **4**(Suppl. 1) (1992),
    776--780.
    [doi:10.1007/BF03180572](https://doi.org/10.1007/BF03180572).

34. []{#reference-34}Mayer Goldberg, “A General Schema for Constructing One-Point Bases in
    the Lambda Calculus,” *BRICS Report Series* **8**(35) (2001), RS-01-35.
    [doi:10.7146/brics.v8i35.21695](https://doi.org/10.7146/brics.v8i35.21695).

35. []{#reference-35}Mario Carneiro, “Formalizing Computability Theory via Partial Recursive
    Functions,” in *10th International Conference on Interactive Theorem
    Proving (ITP 2019)*, LIPIcs **141**, 12:1--12:17.
    [doi:10.4230/LIPIcs.ITP.2019.12](https://doi.org/10.4230/LIPIcs.ITP.2019.12).

36. []{#reference-36}Jian Xu, Xingyuan Zhang, and Christian Urban, “Mechanising Turing
    Machines and Computability Theory in Isabelle/HOL,” in *Interactive
    Theorem Proving (ITP 2013)*, LNCS **7998**, Springer, 2013, 147--162.
    [doi:10.1007/978-3-642-39634-2_13](https://doi.org/10.1007/978-3-642-39634-2_13).

37. []{#reference-37}Yannick Forster, Fabian Kunze, and Maximilian Wuttke, “Verified
    Programming of Turing Machines in Coq,” in *Proceedings of the 9th ACM
    SIGPLAN International Conference on Certified Programs and Proofs
    (CPP 2020)*, ACM, 2020, 114--128.
    [doi:10.1145/3372885.3373816](https://doi.org/10.1145/3372885.3373816).

38. []{#reference-38}Jon Shultis, "A Note on the S Combinator," Technical Report
    CU-CS-304-85, University of Colorado Boulder, 1985.
    [repository record](https://scholar.colorado.edu/concern/reports/qb98mg22g).

39. []{#reference-39}Claude E. Shannon, "A Universal Turing Machine with Two Internal
    States," in *Automata Studies*, Annals of Mathematics Studies **34**,
    Princeton University Press, 1956, 157--165.
    [original scan](https://www.wolframscience.com/prizes/tm23/images/Shannon.pdf).

40. []{#reference-40}Jörg Endrullis, Roel de Vrijer, and Johannes Waldmann, "Local
    Termination: theory and practice," *Logical Methods in Computer
    Science* **6**(3:20) (2010).
    [doi:10.2168/LMCS-6(3:20)2010](<https://doi.org/10.2168/LMCS-6(3:20)2010>).

41. []{#reference-41}Giulio Manzonetto, "A Lambda Calculus Satellite (Invited Talk),"
    in *8th International Conference on Formal Structures for Computation
    and Deduction (FSCD 2023)*, LIPIcs **260**, 3:1--3:14.
    [doi:10.4230/LIPIcs.FSCD.2023.3](https://doi.org/10.4230/LIPIcs.FSCD.2023.3).

42. []{#reference-42}Johannes Waldmann, *The Combinator S*, PhD thesis,
    Friedrich Schiller University Jena, 1998, Part II, Chapter 7 and
    Sections 8.6--9.5.
    [author's thesis](https://www.imn.htwk-leipzig.de/~waldmann/pub/phdthesis/).

43. []{#reference-43}Rick Statman, *Effective Reduction and Conversion Strategies for
    Combinators*, Research Report No. 96-195, Department of Mathematical Sciences,
    Carnegie Mellon University, October 1996.
    [institutional record](https://doi.org/10.1184/r1/6477296).

44. []{#reference-44}Efstathios Zachos, *Kombinatorische Logik und S-Terme*,
    Berichte des Instituts für Informatik **26**, ETH Zürich, 1978,
    Chapter 6, 50--96.
    [doi:10.3929/ethz-a-000136021](https://doi.org/10.3929/ethz-a-000136021).

45. []{#reference-45}Till Dörges, *Unendliche Reduktionen in der kombinatorischen Logik*,
    Diplomarbeit, Universität Leipzig, June 2002.
    [repository record](<https://ul.qucosa.de/landing-page/?tx_dlf[id]=https%3A%2F%2Ful.qucosa.de%2Fapi%2Fqucosa%253A16521%2Fmets>).

46. []{#reference-46}Panagiotis Cheilaris, John Ramirez, and Stathis Zachos, "Checking in
    linear time if an S-term normalizes," 8th Panhellenic Logic Symposium,
    Ioannina, 4--8 July 2011, author-hosted extended version, 1--25.
    [extended version](https://www.corelab.ntua.gr/~philaris/publications/PLS8_submission_23.pdf).

47. []{#reference-47}Aleit Mitschke and Gerd Mitschke, "The Solution of Böhm's S-Problem,"
    unpublished manuscript, TH Darmstadt, 1975.
    [archived manuscript](https://www.wolframcloud.com/obj/combinators/Bibliography/Mitschke-Mitschke-ASolutionofBohmsSProblem).

48. []{#reference-48}Richard Canal, "Complexité de la réduction en logique combinatoire,"
    *RAIRO. Informatique théorique*, 12(4), 339--367, 1978.
    [Numdam full text](https://www.numdam.org/article/ITA_1978__12_4_339_0.pdf).

49. []{#reference-49}Jörg Endrullis, *Term Rewriting Systems, Lecture 9: Strategies*,
    lecture notes, pp. 5, 24, 26--28.
    [Author slides](https://joerg.endrullis.de/trs/09_stategies.pdf/print_09_stategies.pdf),
    accessed 13 September 2026.

50. []{#reference-50}Mousa Mousazadeh, Behrouz Tork Ladani, and Hans Zantema,
    "Liveness Verification in TRSs Using Tree Automata and Termination Analysis,"
    *Computing and Informatics* **29**(3) (2010), 407--426.
    [journal full text](https://www.cai.sk/ojs/index.php/cai/article/download/91/74/331).

51. []{#reference-51}Martin Kutrib and Uwe Meyer, "Tree-Walking-Storage Automata,"
    in *Developments in Language Theory (DLT 2023)*, LNCS **13911**,
    182--194, 2023; first online 19 May 2023.
    [doi:10.1007/978-3-031-33264-7_15](https://doi.org/10.1007/978-3-031-33264-7_15).

52. []{#reference-52}Nathan Douglas, *CombinatorCalculusPlayground*, source-code repository,
    commit `5a7d3a25014983ec3bebef3eb9870933585209cb`, 25 August 2026,
    accessed 14 September 2026.
    [Pinned source](https://github.com/ndouglas/CombinatorCalculusPlayground/tree/5a7d3a25014983ec3bebef3eb9870933585209cb).
    Relevant declarations: `Simulation` and `PathEncoding` in
    `Universality/Defs.lean`; `no_pathEncoding_SK_pureS` in
    `Universality/Calibration.lean`; `steps_decidable_of_kFree` in
    `Decidability.lean`, under `CombinatorCalculusPlayground/`.

53. []{#reference-53}Jan Willem Klop, Vincent van Oostrom, and Femke van Raamsdonk,
    "Reduction Strategies and Acyclicity," in *Rewriting, Computation and
    Proof*, LNCS **4600**, 89--112, 2007.
    [doi:10.1007/978-3-540-73147-4_5](https://doi.org/10.1007/978-3-540-73147-4_5).
    [Author manuscript](https://www.cs.vu.nl/~femke/ps/jpj60.pdf).

...
$$
*(where the time horizon $\mathcal{T}$ is implicitly related to the complexity of achieving precision $\delta_{\rm SPAP}$)*. Because the required complexity diverges according to these bounds as $\alpha \to \alpha_{SPAP}^-$, attaining performance arbitrarily close to the fundamental SPAP limit is physically unattainable, requiring unbounded resources.
...
This direct coupling between the cost of motion (acceleration) and the cost of prediction (complexity) is formalized in the Unified Cost of Transgression (UCT) theorem, derived in Appendix N. The UCT shows that the total work required for any process is a sum of the kinetic work and the predictive work, where the latter is explicitly dependent on both the proximity to the SPAP limit and the acceleration profile of the trajectory.

...
| Appendix T matching-scale observables | Conditional on threshold data, running, matching, and the quantitative constructions introduced there; Remark T.17a.4 and Proposition T.17a.5 rule out the sector-independent local affine truncation, while Theorem T.78.2 records that the current canonical ledger does not yet determine the branch data needed to compute the operative global sector-resolving $\mathrm{MS2}_{\mu_G}$ block functional on $\widetilde X$ |

...
**Remark 14.2 (Layered Causality Protection).** The framework's causality protection operates through four layers, all rooted in SPAP and the irreducible cycle cost $\varepsilon \ge \ln 2$ (Theorem 31):
...
(ii) *Thermodynamic layer*: every processing attempt produces irreversible entropy $\Sigma \ge \varepsilon \ge \ln 2$ per cycle (Theorem 31), advancing the system's arrow and, in the fluctuation-relation regime of Theorem O.3, exponentially suppressing reversed trajectories.
...
*   **Emergence Rigor:** The geometric part of the discrete-to-effective-continuum bridge is now packaged on the $M=24$, $D=4$ operational-continuum branch by Theorem 43.5: Lemma C.6d gives the $D_4$ shell moments, Theorem C.6e gives PCE shell isotropy and noncollapse, Theorem C.6c gives the noncollapsed $\mathrm{RCD}^*(K,4)$ bridge under the uniform curvature-transfer hypothesis, Theorem D.6e gives the Mosco–Cheeger energy closure under its finite-core/recovery hypotheses, and Theorem 44a gives the regular-branch manifold structure under its quantitative Euclidean-rigidity input. This does not make an exact real-number continuum physically real; by Theorem K.10.3a the continuum remains the finite-resolution effective closure of the discrete substrate. The remaining AQFT-side gap is a controlled coarse-graining theorem for local Hamiltonians strong enough either to retain the continuum Hamiltonian or to prove its suppression/cancellation. The source tensor itself is already identified by the coarse-graining / variational-identification / direct-conservation chain of Theorems B.8b–B.8d together with Theorem X.5a; what remains open is the local-Hamiltonian convergence problem, not the identity of $T_{\mu\nu}^{(MPU)}$.
*   **Parameter Determination:** The manuscript separates **threshold variables** from **scaling variables** (Convention P.14.1a; Corollary P.14.1b). Threshold variables are discrete structural quantities fixed on a stated branch, such as $(K_0,d_0,a,b,M,k,D)$ and, within its own anomaly+CP class, $N_g=3$ (Proposition R.3.5f). Scaling variables include RG / matching scales $(\mu_G,\mu_\lambda)$, residual matching shifts $\delta_i$, vacuum prefactors $(A_{\mathrm{eff}},K,N_{\mathrm{eff}})$, and environment- or coarse-graining-dependent quantities such as $(\alpha,\beta)$, $C_{scale}$, $\Gamma_0$, and the resource-scarcity parameter of Definition 20. The lifted gauge-threshold tuple $(\Delta_1,\Delta_2,\Delta_3)$ has a canonically fixed local part (Proposition T.17a.3a); its completed values are determined only after the explicit global flag-lift spectral problem is solved (Theorems T.69 and T.78). Once those spectral values and the residual flavor-profile data are fixed, the quantitative Appendix-T parameter vector $\Pi_T$ is uniquely determined (Theorems T.77 and T.79). Distinguishing threshold roles from scaling roles is therefore part of the formal ontology of the parameter sector.
...
This provides an information-theoretic answer to “why 3+1 dimensions?” within that model: because $K(4) = 24$ and no other dimension satisfies $K(D) = 2ab$ with $a = 2$, $b = 6$, $d_0 = 8$. The question becomes: why does PCE at $\varepsilon = \ln 2$ produce exactly 24 interface modes? Given the Appendix Z mode-channel identification, $D = 4$ follows by arithmetic.
...
Appendix C.7 makes the present status boundary explicit. Appendix C proves that large-scale geometric irregularity is penalized by the viability requirements (LV), (GC), and (RE), and Theorem C.6 packages this as conditional coarse-grained doubling and local $(1,2)$-Poincaré control under its stated hypotheses. Lemma C.6d and Theorem C.6e add the $D_4$ first-shell isotropy and noncollapse closure on the $M=24$, $D=4$ branch; Theorem C.6c supplies the noncollapsed $\mathrm{RCD}^*(K,4)$ bridge under the uniform $\mathrm{BE}(K,4)$ curvature-transfer condition; Theorem D.6e supplies Mosco/quadratic limit-energy identification under its finite-core/recovery hypotheses; and Theorem 44a supplies the regular-branch Euclidean-rigidity conclusion under its quantitative input. What remains separate is the AQFT convergence layer of Theorem F.0, especially controlled coarse-graining of the local Hamiltonian sector. The local-horizon thermodynamic bridge used in Section 12 is formalized separately in Theorem 48a. For nearby mathematical literature on graph Poincaré inequalities and split-type properties in lattice systems, see [Levi et al. 2023; Matsui 2013; Naaijkens & Ogata 2022].
...
| Operational continuum manifold / Lorentzian geometry | Conditional on the $M=24$, $D=4$ shell branch packaged by Theorem 43.5, using Appendix C.6d–C.6e, Theorem C.6c, Theorem D.6e, and Theorem 44a under their stated curvature-transfer, finite-core/recovery, and quantitative-rigidity hypotheses; exact continuum ontology excluded by Theorem K.10.3a |
| AQFT route | Conditional on the operational-continuum branch plus the controlled coarse-graining and compatible-embedding hypotheses of Theorem F.0; lightcone normalization is supplied by Corollary F.1a on the strict finite-range single-clock ND-RID branch and remains explicit otherwise |
| Einstein route / field equations | Conditional on Theorem 43, the explicit Section 11.4 bridge package, Theorem 49, Postulate 4, and the local-horizon bridge of Theorem 48a |

...
*   **Emergence Rigor:** Demonstrating rigorous convergence from the discrete MPU network to continuum QFT/GR still requires several theorem-sized bridge results. Beyond proving Theorem 43 dynamically and fully integrating the local-horizon KMS/Clausius bridge formalized in Theorem 48a, the remaining AQFT-side gap is a controlled coarse-graining theorem for local Hamiltonians strong enough either to retain the continuum Hamiltonian or to prove its suppression/cancellation. The source tensor itself is already identified by the coarse-graining / variational-identification / direct-conservation chain of Theorems B.8b–B.8d together with Theorem X.5a; what remains open is the local-Hamiltonian convergence problem, not the identity of $T_{\mu\nu}^{(MPU)}$.
...
Sections 11 and 12 are explicitly conditional on the regularity and convergence hypotheses recorded in Appendices C and D. Section 11.4 additionally assumes the weighted-shell/local-isotropy input, or an equivalent replacement, a radius-2 curvature-transfer theorem placing the rescaled family in the synthetic-curvature class used there, Mosco/quadratic limit-energy identification, and the branch-appropriate Euclidean-rigidity input. Appendix F adds a separate AQFT layer with its own bounded-degree/short-range, uniform Lieb-Robinson, controlled coarse-graining with coherent Hamiltonian control, compatible-embedding, and lightcone-identification hypotheses through Theorem F.0. The Einstein-equation step of Section 12 uses local thermodynamic equilibrium through Postulate 4, with the local-horizon KMS/Clausius bridge formalized in Theorem 48a under the Appendix F/G hypotheses. Theorem 43 is therefore best read as the manuscript's formal regularity theorem; the later manifold, AQFT, and gravity conclusions invoke the additional bridge assumptions explicitly where they are needed.
...
Appendix C.7 makes the present status boundary explicit. Appendix C proves that large-scale geometric irregularity is penalized by the viability requirements (LV), (GC), and (RE), and Theorem C.6 packages this as conditional coarse-grained doubling and local $(1,2)$-Poincaré control under its stated hypotheses. What Appendix C does not by itself provide is the weighted-shell/local-isotropy closure, the radius-2 curvature transfer and non-collapse package, the Mosco/quadratic limit-energy identification, or the Euclidean rigidity input used in Section 11.4, nor the AQFT convergence hypotheses of Theorem F.0. The local-horizon thermodynamic bridge used in Section 12 is formalized separately in Theorem 48a. For nearby mathematical literature on graph Poincaré inequalities and split-type properties in lattice systems, see [Levi et al. 2023; Matsui 2013; Naaijkens & Ogata 2022].
...
| Continuum manifold / Lorentzian geometry | Conditional on Theorem 43 and the explicit Section 11.4 bridge package: weighted-shell/local-isotropy, or an equivalent replacement, radius-2 curvature transfer, Mosco/quadratic limit-energy identification, and branch-appropriate Euclidean rigidity |
| AQFT route | Conditional on Theorem 43, the explicit Section 11.4 bridge package, and the additional hypotheses of Theorem F.0 |

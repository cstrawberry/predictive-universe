...
**Canonical-branch evaluation (Theorem Y.9):** On the Appendix Y branch, the baryogenesis complexity $\kappa_B=\kappa_{EW}/2+\varepsilon/N_g=19.48$ yields:
$$
...
Appendix T likewise sits on top of the fixed backbone. Proposition T.59 closes the core counting chain. In the lifted threshold sector, Remark T.17a.4 and Proposition T.17a.5 provide a theorem-level local no-go for sector-independent affine truncations, while Definition T.17a, Convention T.69a, Theorem T.70, Corollary T.69.1, and Theorem T.78 define the global sector-resolving spectral calculation on $\widetilde X$. Theorem T.78.2 makes the complementary status boundary explicit, and Theorem T.78.5 closes that boundary negatively for the current framework: the canonical ledger supplies no PU-internal spectral branch package executing the global block sum. The later quantitative hierarchy results therefore continue to depend on the threshold data, prefactors, running, matching prescriptions, and any explicitly appended branch data introduced in those sections.
...
| Appendix T matching-scale observables | Conditional on threshold data, running, matching, and the quantitative constructions introduced there; Remark T.17a.4 and Proposition T.17a.5 rule out the sector-independent local affine truncation, while Theorem T.78.5 proves that the current canonical ledger supplies no PU-internal branch package determining the operative global sector-resolving $\mathrm{MS2}_{\mu_G}$ block functional on $\widetilde X$ |

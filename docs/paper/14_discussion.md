...
- **Two-State Vector Formalism (TSVF):** Developed by Aharonov, Bergmann, and Lebowitz [Aharonov et al. 1964], this approach describes quantum systems using both forward-evolving and backward-evolving state vectors and is useful for pre- and post-selected ensembles and weak-measurement calculations [Aharonov & Vaidman 2008]. PU does not need to add a TSVF future-state ontology: the formalism's operational content is absorbed as finite post-selected conditioning of ND-RID process tensors (Corollary M.6.14d), while only the ontic reading in which a future boundary acts as a present causal input is excluded.
...
PU instead supplies several conditional finite-model ledgers. Refresh/minorization can give a transfer-mixing gap through Lemma E.1 and Theorem E.2; the accepted rootless branch gives a dimensionless lattice norm gap, since Theorem Z.8c identifies $\Lambda_{24}$ as the unique rootless even unimodular rank-$24$ lattice and Theorem Z.8g supplies the conditional energy--norm normalization on $\mathfrak B_{\mathrm{mass}}$; and Corollary Q.0.1's action-per-update ledger provides a scale only after duration and spectral maps are registered. None is a continuum Yang–Mills mass gap.
...
The current archive defines no theory interval or agreement pull. Numerical promotion requires an accepted Appendix Y source, transport, freeze-out, washout, normalization, residual, and covariance certificate fixed before comparison with $\eta_B^{\mathrm{obs}}=(6.12\pm0.04)\times10^{-10}$.
...
**Route 2 (Geometric).** QFI isotropy at the PCE-Attractor (Theorem Z.5) fixes the ambient QFI inner product on the 24-dimensional mode space. Imposing rootlessness as an explicit geometric branch criterion, Theorem Z.8c identifies the unique compatible even unimodular lattice as the Leech lattice $\Lambda_{24}$



$$g_{QFI}=I_{24},\;\text{rootless}\xrightarrow{\text{Thm Z.8c}}\Lambda_{24}\implies |v|_{min}=2,$$
a dimensionless norm gap; $\Delta_{mass}=2\mu_0^{alg}$ additionally requires $\mathfrak B_{mass}$ and $\gamma=1$.
...
(2,6,12,24)
$$
is exact arithmetic after the independently obtained branch values are inserted. The inclusion
$$
